...
MLflow automatically records the `artifact_uri` property as a part of [`mlflow.entities.RunInfo`](/docs/3.15.1/api_reference/python_api/mlflow.entities.html#mlflow.entities.RunInfo) so that you can retrieve the location of the artifacts for historical runs using the [`mlflow.get_artifact_uri()`](/docs/3.15.1/api_reference/python_api/mlflow.html#mlflow.get_artifact_uri) API. Also, `artifact_location` is a property recorded on [`mlflow.entities.Experiment`](/docs/3.15.1/api_reference/python_api/mlflow.entities.html#mlflow.entities.Experiment) for setting the default location to store artifacts for all runs for models within a given experiment.
...
If the artifacts are not stored directly on supported storage (for example, with [proxied artifact access](/docs/3.15.1/self-hosting/architecture/tracking-server.md#tracking-server-artifact-store), the default configuration), the UI falls back to downloading the artifact through the Tracking Server.
