...
![OpenTelemetry](/docs/3.6.0/images/llms/tracing/opentelemetry/hero.png)
...
To get started with vendor-neutral tracing quickly, you can use the OpenTelemetry-native MLflow Tracing SDK. The SDK provides a convenient one-line auto-tracing experience for [popular GenAI libraries](/docs/3.6.0/genai/tracing/integrations.md) and enhances general OpenTelemetry traces with rich AI-specific metadata such as prompts, token usage, model name, etc. See [Quickstart](/docs/3.6.0/genai/tracing/quickstart/python-openai.md) to get started with the MLflow Tracing SDK.
...
See [Collect OpenTelemetry Traces into MLflow](/docs/3.6.0/genai/tracing/opentelemetry/ingest.md) for more details on how to collect traces into MLflow Server.
...
MLflow traces and metrics can be exported to other OpenTelemetry-compatible backends such as Datadog, Grafana, Prometheus, etc., to integrate with your existing observability platform. You can also use [dual export](/docs/3.6.0/genai/tracing/opentelemetry/export.md#dual-export) to send traces to both MLflow and an OpenTelemetry-compatible backend simultaneously.

See [Export MLflow Traces/Metrics via OTLP](/docs/3.6.0/genai/tracing/opentelemetry/export.md) for more details.
