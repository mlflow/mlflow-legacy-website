...
MLflow provides automatic tracing (autolog) for 20+ popular libraries. See the complete list at [Automatic Tracing Integrations](/docs/3.6.0/genai/tracing/integrations.md).
...
2. You call [`mlflow.search_traces()`](/docs/3.6.0/api_reference/python_api/mlflow.html#mlflow.search_traces)
...
You can customize what appears in the Request and Response columns of the trace list using [`mlflow.update_current_trace()`](/docs/3.6.0/api_reference/python_api/mlflow.html#mlflow.update_current_trace):
...
To register a custom hooks, use the [`mlflow.tracing.configure`](/docs/3.6.0/api_reference/python_api/mlflow.tracing.html#mlflow.tracing.configure) API. For example, the following code filters out the document contents from the retriever span output to reduce the trace size:
...
Refer to the [Redacting Sensitive Data Safe](/docs/3.6.0/genai/tracing/observe-with-traces/masking.md) guide for more details about the hook API and examples.
...
To **disable** tracing, [`mlflow.tracing.disable()`](/docs/3.6.0/api_reference/python_api/mlflow.tracing.html#mlflow.tracing.disable) API will cease the collection of trace data from within MLflow and will not log any data to the MLflow Tracking service regarding traces.

To **enable** tracing (if it had been temporarily disabled), [`mlflow.tracing.enable()`](/docs/3.6.0/api_reference/python_api/mlflow.tracing.html#mlflow.tracing.enable) API will re-enable tracing functionality for instrumented models that are invoked.
...
You can delete traces using the [`mlflow.client.MlflowClient.delete_traces()`](/docs/3.6.0/api_reference/python_api/mlflow.client.html#mlflow.client.MlflowClient.delete_traces) method:
...
Read more about [trace deletion](/docs/3.6.0/genai/tracing/observe-with-traces/delete-traces.md).
...
For production monitoring, see [Production Tracing](/docs/3.6.0/genai/tracing/prod-tracing.md) for integration patterns.
...
Yes, MLflow provides comprehensive manual tracing capabilities. Please refer to the [Manual Tracing](/docs/3.6.0/genai/tracing/app-instrumentation/manual-tracing.md) guide for detailed information on creating traces and spans manually using decorators, context managers, and low-level APIs.
...
**Documentation**: Start with the [MLflow Tracing documentation](/docs/3.6.0/genai/tracing.md)
...
*For additional questions or issues not covered here, please check the [MLflow documentation](/docs/3.6.0/genai/tracing.md) or reach out to the community.*
