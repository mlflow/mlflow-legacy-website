...
The Span object is a fundamental building block in the [Trace](/docs/3.16.0/genai/concepts/trace.md) data model. It is a container for the information about the individual steps of the trace, such as LLM calls, tool execution, retrieval, etc. Spans form a hierarchical tree structure within a single trace, which represents the execution flow of the trace.

![Example of a Span for a Tool Calling Agent](/docs/3.16.0/images/llms/tracing/schema/span_architecture.png)
...
| `inputs`        | `Optional[Any]`   | The input data that is passed into the particular stage of your application. May contain [Attachment](/docs/3.16.0/genai/tracing/observe-with-traces/multimodal.md#trace-attachments) objects for binary content, which are automatically replaced with reference URIs.    |
| `outputs`       | `Optional[Any]`   | The output data that is passed out of the particular stage of your application. May contain [Attachment](/docs/3.16.0/genai/tracing/observe-with-traces/multimodal.md#trace-attachments) objects for binary content, which are automatically replaced with reference URIs. |
...
![Search Spans by Type](/docs/3.16.0/assets/images/search_spans_by_type-ddb1e2356a4b8ba83b6cb59c04fffc89.png)
...
![Span links shown in the trace explorer Links tab](/docs/3.16.0/assets/images/span-links-tab-2f28f4c635cb0b8b550c0cf85f50780c.png)

For detailed usage patterns and examples, see [Manual Tracing — Span Links](/docs/3.16.0/genai/tracing/app-instrumentation/manual-tracing.md#span-links).
