...
In addition to the [Auto Tracing](/docs/3.16.0/genai/tracing/app-instrumentation/automatic.md) integrations, you can instrument your LLM application or AI agent by using MLflow's manual tracing APIs.

Prefer autolog when your framework is supported

If your app is built on a [supported framework](/docs/3.16.0/genai/tracing/integrations.md) (LangChain, LangGraph, OpenAI, LlamaIndex, and 40+ others), call that framework's `autolog()` once instead of decorating individual functions by hand. Autolog captures the framework's internal calls as a single connected trace. Hand-decorating each function separately produces one independent root trace per function rather than a unified trace, and misses the internal spans autolog records. Use manual tracing to add detail that autolog does not capture, or to instrument code that no integration covers.
...
![Tracing Decorator](/docs/3.16.0/assets/images/trace-decorator-8ae22208121b562582947549f8b9a46e.png)
...
* 🎯 **`span_type` parameter** to set the type of span. Set either one of built-in [Span Types](/docs/3.16.0/genai/concepts/span.md#span-types) or a string.
...
Use the [`mlflow.get_current_active_span()`](/docs/3.16.0/api_reference/python_api/mlflow.html#mlflow.get_current_active_span) API.
...
Tags can be added to traces to provide additional metadata at the trace level. There are a few different ways to set tags on a trace. Please refer to the [how-to guide](/docs/3.16.0/genai/tracing/attach-tags.md) for the other methods.
...
See [Multimodal Content and Attachments in Traces](/docs/3.16.0/genai/tracing/observe-with-traces/multimodal.md#manual-tracing) for more examples.
...
Use the [`mlflow.start_span()`](/docs/3.16.0/api_reference/python_api/mlflow.html#mlflow.start_span) context manager.
...
[Span Links](/docs/3.16.0/genai/concepts/span.md#span-links) connect related spans — within the same trace or across different traces — without establishing a parent-child relationship. This is useful for tracking causal relationships in multi-agent systems, async workflows, and retry chains.
...
[](/docs/3.16.0/images/llms/tracing/trace-exception.mp4)
...
Of course, we recommend using the [auto-tracing for OpenAI](/docs/3.16.0/genai/tracing/integrations/listing/openai.md) for examples like this, which does the same job but with one-liner code. The example below is for demonstration purposes.
...
![Multi threaded tracing](/docs/3.16.0/assets/images/tracing-multi-thread-acfb6f382ce45e030c95b7c0536749bf.png)
...
### [Combining with Auto-Tracing](/docs/3.16.0/genai/tracing/app-instrumentation/automatic.md#combining-manual-and-automatic-tracing)

[Mix automatic and manual tracing for optimal observability](/docs/3.16.0/genai/tracing/app-instrumentation/automatic.md#combining-manual-and-automatic-tracing)

[Learn more →](/docs/3.16.0/genai/tracing/app-instrumentation/automatic.md#combining-manual-and-automatic-tracing)

### [Trace Concepts](/docs/3.16.0/genai/concepts/trace.md)

[Understand the structure and components of MLflow traces](/docs/3.16.0/genai/concepts/trace.md)

[Explore concepts →](/docs/3.16.0/genai/concepts/trace.md)

### [Querying Traces](/docs/3.16.0/genai/tracing/search-traces.md)

[Programmatically search and analyze your traces](/docs/3.16.0/genai/tracing/search-traces.md)
