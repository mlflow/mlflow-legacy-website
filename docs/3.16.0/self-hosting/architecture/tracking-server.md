...
* **Sharing Results**: The tracking server also serves [Tracking UI](/docs/3.16.0/ml/tracking.md#tracking_ui) endpoint, where team members can easily explore each other's results.
...
Once the tracking server is started, connect your local clients by setting the `MLFLOW_TRACKING_URI` environment variable to the server's URI, along with its scheme and port (for example, `http://10.0.0.1:5000`) or call [`mlflow.set_tracking_uri()`](/docs/3.16.0/api_reference/python_api/mlflow.html#mlflow.set_tracking_uri).

MLflow APIs like [`mlflow.start_run()`](/docs/3.16.0/api_reference/python_api/mlflow.html#mlflow.start_run), [`mlflow.log_param()`](/docs/3.16.0/api_reference/python_api/mlflow.html#mlflow.log_param), [`mlflow.start_trace()`](/docs/3.16.0/api_reference/python_api/mlflow.html#mlflow.start_trace) make API requests to your remote tracking server and log the data.
...
This section describes how to configure the tracking server for some common use cases. The section requires you to have a basic knowledge about the tracking server architecture, please visit [Architecture Overview](/docs/3.16.0/self-hosting/architecture/overview.md) if you are not familiar with it yet.
...
| Isolate experiments, models, and prompts | `--enable-workspaces`                                   | [Getting Started with Workspaces](/docs/3.16.0/self-hosting/workspaces/getting-started.md) |
...
See [Archive Traces](/docs/3.16.0/genai/tracing/observe-with-traces/archive-traces.md) for the end-user workflow and archival failure / retry behavior, and [Getting Started with Workspaces](/docs/3.16.0/self-hosting/workspaces/getting-started.md) for workspace-level examples.
...
If you are using remote storage, you have to configure the credentials for the server to access the artifacts. Be aware of that The MLflow artifact proxied access service enables users to have an *assumed role of access to all artifacts* that are accessible to the Tracking Server. Refer [Manage Access](/docs/3.16.0/self-hosting/architecture/artifact-store.md#artifacts-stores-manage-access) for further details.
...
For this reason, the client needs direct access to the artifact store. For instructions on setting up these credentials, see [Artifact Stores documentation](/docs/3.16.0/self-hosting/architecture/artifact-store.md#artifacts-stores-manage-access).
...
MLflow 3.5.0+ includes built-in security middleware to protect against DNS rebinding and CORS attacks. When using `--host 0.0.0.0`, configure the `--allowed-hosts` option to specify which domains can access your server. See [Security Configuration](/docs/3.16.0/self-hosting/security/network.md) for details.
...
For detailed configuration options, see [Security Configuration](/docs/3.16.0/self-hosting/security/network.md).
...
For notebook integration and UI embedding options, see [Network Security](/docs/3.16.0/self-hosting/security/network.md) configuration.
