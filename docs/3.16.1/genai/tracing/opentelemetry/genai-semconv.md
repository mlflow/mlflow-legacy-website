...
![GenAI Semantic Conventions Trace in MLflow](/docs/3.16.1/images/llms/tracing/opentelemetry/genai-semconv-trace.png)
...
For the full mapping tables, see the [Attribute Mapping Reference](/docs/3.16.1/genai/tracing/opentelemetry/attribute-mapping.md).
...
See [Collect OpenTelemetry Traces into MLflow](/docs/3.16.1/genai/tracing/opentelemetry/ingest.md) for setup instructions.
...
On the ingestion side, MLflow recognizes GenAI semconv traces from any compliant tool, including [Google ADK](/docs/3.16.1/genai/tracing/integrations/listing/google-adk.md), [LiveKit Agents](/docs/3.16.1/genai/tracing/integrations/listing/livekit.md), [Spring AI](/docs/3.16.1/genai/tracing/integrations/listing/spring-ai.md), and more.

## Dual Export with GenAI Format[​](#dual-export-with-genai-format "Direct link to Dual Export with GenAI Format")

You can combine GenAI semconv export with [dual export](/docs/3.16.1/genai/tracing/opentelemetry/export.md#dual-export) to send traces to both MLflow and an OpenTelemetry-compatible backend simultaneously. MLflow stores traces in its own format locally (for the MLflow UI) while exporting the GenAI semconv version to the OTLP endpoint. To enable dual export, set `MLFLOW_TRACE_ENABLE_OTLP_DUAL_EXPORT=true`.
