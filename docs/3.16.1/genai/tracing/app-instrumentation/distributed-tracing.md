...
![Distributed trace in the MLflow UI showing spans from both client and server](/docs/3.16.1/images/llms/tracing/distributed-tracing-sample.png)
...
* Use [get\_tracing\_context\_headers\_for\_http\_request](/docs/3.16.1/api_reference/python_api/mlflow.tracing.html#mlflow.tracing.get_tracing_context_headers_for_http_request) in the **client side** (caller service) to get headers containing the current trace context.
* Use [set\_tracing\_context\_from\_http\_request\_headers](/docs/3.16.1/api_reference/python_api/mlflow.tracing.html#mlflow.tracing.set_tracing_context_from_http_request_headers) in the **server side** (callee service) to extract the trace and span information from the incoming request headers.

![Distributed tracing flow](/docs/3.16.1/assets/images/distributed-tracing-flow-be4470321832a938323b4a3f9960e3b0.png)
...
The gateway endpoint must have [usage tracking](/docs/3.16.1/genai/governance/ai-gateway/usage-tracking.md) enabled for both the gateway trace and the distributed span to be created.
