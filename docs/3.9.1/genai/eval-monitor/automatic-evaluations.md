...
[](/docs/3.9.1/images/llms/tracing/automatic-evaluation-ui-setup.mp4)
...
| ---------------- | ------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------- |
| **When it runs** | Automatically, as traces and conversations are logged                                | Manually, when you call [`mlflow.genai.evaluate()`](/docs/3.9.1/genai/eval-monitor/running-evaluation/traces.md) |
...
1. **[The MLflow Server is running](/docs/3.9.1/genai/getting-started/connect-environment.md)**
2. **[MLflow Tracing is enabled](/docs/3.9.1/genai/tracing/quickstart.md)** in your agent or LLM application
   <!-- -->
   * **For [multi-turn conversation evaluation](#session-level-evaluation)**, traces must include [session IDs](/docs/3.9.1/genai/tracing/track-users-sessions.md)
3. **[An AI Gateway endpoint is configured](/docs/3.9.1/genai/governance/ai-gateway/endpoints/create-and-manage.md)** for LLM judge execution
   <!-- -->
   * LLM judges require an LLM to perform evaluations, and [AI Gateway endpoints](/docs/3.9.1/genai/governance/ai-gateway/endpoints/create-and-manage.md) provide secure, managed access to LLMs

## Setting Up Automatic Evaluation[​](#setting-up-automatic-evaluation "Direct link to Setting Up Automatic Evaluation")

These examples show how to set up LLM judges that automatically evaluate traces and multi-turn conversations as they're logged to an [MLflow Experiment](/docs/3.9.1/genai/tracing/quickstart.md#create-a-mlflow-experiment), and how to update or disable existing judges. For more details on creating LLM judges, see [LLM-as-a-Judge](/docs/3.9.1/genai/eval-monitor/scorers.md#llms-as-judges).

note

* Automatic evaluation only supports [LLM judges](/docs/3.9.1/genai/eval-monitor/scorers.md#llms-as-judges). Code-based scorers (using the [`@scorer` decorator](/docs/3.9.1/genai/eval-monitor/scorers/custom.md)) are not supported. Use [built-in judges](/docs/3.9.1/genai/eval-monitor/scorers/llm-judge/predefined.md) or create custom judges with [`make_judge()`](/docs/3.9.1/genai/eval-monitor/scorers/llm-judge/custom-judges/create-custom-judge.md).
...
1. Navigate to your experiment and select the **Judges** tab
   ...
   ![Judges tab](/docs/3.9.1/assets/images/judges-tab-c1dea9a24e9809d6c7f8dbb4e2891767.png)

2. Click **+ New LLM judge**

   ![New LLM judge button](/docs/3.9.1/assets/images/new-llm-judge-button-ee8d6028b0f3b278dbf49cd81759de7f.png)
...
4. **Configure the judge**:
   ...
   * **Model**: Select an [AI Gateway](/docs/3.9.1/genai/governance/ai-gateway/endpoints/create-and-manage.md) endpoint (LLM) to run the judge
...
5. **Evaluation settings**:
   ...
   * Optionally add a **Filter string** to target specific traces or sessions
   ...
   ![Evaluation settings](/docs/3.9.1/assets/images/evaluation-settings-015320f1e469372c39ed925295791f40.png)

6. Click **Save**

7. To edit or disable an existing judge, select it in the **Judges** tab.

   ![Edit LLM judge button](/docs/3.9.1/assets/images/edit-llm-judge-button-20eae3b628d868a4c3f014c5e478701f.png)

For more details about the APIs used in this example, see [`mlflow.genai.scorers.Scorer.start()`](/docs/3.9.1/api_reference/python_api/mlflow.genai.html#mlflow.genai.scorers.Scorer.start), [`mlflow.genai.scorers.Scorer.update()`](/docs/3.9.1/api_reference/python_api/mlflow.genai.html#mlflow.genai.scorers.Scorer.update), and [`mlflow.genai.scorers.Scorer.stop()`](/docs/3.9.1/api_reference/python_api/mlflow.genai.html#mlflow.genai.scorers.Scorer.stop).
...
Assessments from automatic evaluation appear directly in the MLflow UI. For traces, assessments typically appear within a minute or two of logging. Multi-turn sessions are evaluated after 5 minutes of inactivity (no new traces have been added to the session) by default—this is [configurable](/docs/3.9.1/api_reference/python_api/mlflow.environment_variables.html#mlflow.environment_variables.MLFLOW_ONLINE_SCORING_DEFAULT_SESSION_COMPLETION_BUFFER_SECONDS).

Navigate to your experiment in the MLflow UI to see results.

![Online evaluation charts showing assessment trends](/docs/3.9.1/assets/images/online-evaluation-charts-c92ee3860b1b862ab09f32df26f2bd5d.png)

Charts in the Overview tab display quality and performance trends over time

![Online evaluation results showing assessment scores for traces](/docs/3.9.1/assets/images/online-evaluation-results-ed55cb9aec590cdc084af7f3221e8db5.png)
...
Use [trace search syntax](/docs/3.9.1/genai/tracing/search-traces.md) to target specific traces. Examples:
...
* **Session completion**: A session is considered complete (ready for automatic evaluation) after no new traces arrive for 5 minutes ([configurable](/docs/3.9.1/api_reference/python_api/mlflow.environment_variables.html#mlflow.environment_variables.MLFLOW_ONLINE_SCORING_DEFAULT_SESSION_COMPLETION_BUFFER_SECONDS))
...
For more information about session evaluation, see [Evaluate Conversations](/docs/3.9.1/genai/eval-monitor/running-evaluation/multi-turn.md).
...
The [MLflow Server](/docs/3.9.1/genai/getting-started/connect-environment.md) uses [AI Gateway](/docs/3.9.1/genai/governance/ai-gateway.md) endpoints to access LLMs for judge execution, ensuring secure and managed model access. Only the relevant trace or session data required by the judge (such as inputs, outputs, and context) is sent to the LLM.
...
| **Unexpected or unsatisfactory judge results** | Edit the judge's instructions or use the [`align()` method](/docs/3.9.1/genai/eval-monitor/scorers/llm-judge/alignment.md) to optimize them automatically |
...
For further debugging, enable debug logging on the MLflow server by setting the [`MLFLOW_LOGGING_LEVEL=DEBUG`](/docs/3.9.1/api_reference/python_api/mlflow.environment_variables.html#mlflow.environment_variables.MLFLOW_LOGGING_LEVEL) environment variable and checking the MLflow server logs.

## Next Steps[​](#next-steps "Direct link to Next Steps")

### [LLM-as-a-Judge](/docs/3.9.1/genai/eval-monitor/scorers.md#llms-as-judges)

[Learn more about creating and customizing LLM judges for your specific quality criteria.](/docs/3.9.1/genai/eval-monitor/scorers.md#llms-as-judges)

[Learn more →](/docs/3.9.1/genai/eval-monitor/scorers.md#llms-as-judges)

### [Evaluate Conversations](/docs/3.9.1/genai/eval-monitor/running-evaluation/multi-turn.md)

[Learn more about evaluating multi-turn conversations.](/docs/3.9.1/genai/eval-monitor/running-evaluation/multi-turn.md)

[Learn more →](/docs/3.9.1/genai/eval-monitor/running-evaluation/multi-turn.md)
