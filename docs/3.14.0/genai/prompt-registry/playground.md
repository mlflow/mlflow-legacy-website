...
The **LLM Playground** is an in-browser chat surface for testing [MLflow AI Gateway](/docs/3.14.0/genai/governance/ai-gateway.md) endpoints and prompts from the [MLflow Prompt Registry](/docs/3.14.0/genai/prompt-registry.md) without writing any code. Iterate on a system message, dial in sampling parameters, attach tools, request structured output, and replay multi-turn conversations, all from a single page in the MLflow UI.
...
Tune temperature, max\_tokens, top\_p, top\_k, presence/frequency penalties, and stop sequences per request. Leave a field blank to use the provider's default.
...
Load any registered prompt version into the conversation with one click. A stored model config (model, temperature, max\_tokens, response format) is applied automatically.

## Prerequisites[​](#prerequisites "Direct link to Prerequisites")

Before opening the Playground, you need an **AI Gateway chat endpoint**. Follow [Create and Manage Endpoints](/docs/3.14.0/genai/governance/ai-gateway/endpoints/create-and-manage.md) to register one. The Playground submits requests against the `/gateway/mlflow/v1/chat/completions` route.
...
![MLflow LLM Playground overview](/docs/3.14.0/images/llms/playground/playground-overview.png)
...
## Loading prompts from the Prompt Registry[​](#loading-prompts-from-the-prompt-registry "Direct link to Loading prompts from the Prompt Registry")

Click **Load prompt from registry** in the top bar to pick from prompts registered in the [Prompt Registry](/docs/3.14.0/genai/prompt-registry.md):
...
* [AI Gateway](/docs/3.14.0/genai/governance/ai-gateway.md) — create and manage the endpoints the Playground talks to.
* [Prompt Registry](/docs/3.14.0/genai/prompt-registry.md) — register and version the prompts you load into the Playground.
