...
![LangGraph Tracing via autolog](/docs/3.14.0/images/llms/tracing/langgraph-agent-trace.png)
...
[MLflow Tracing](/docs/3.14.0/genai/tracing.md) provides automatic tracing capability for LangGraph, as a extension of its LangChain integration. By enabling auto-tracing for LangChain by calling the [`mlflow.langchain.autolog()`](/docs/3.14.0/api_reference/python_api/mlflow.langchain.html#mlflow.langchain.autolog) function, MLflow will automatically capture the graph execution into a trace and log it to the active MLflow Experiment. In TypeScript, you can pass the MLflow LangChain callback to the `callbacks` option.
...
Start the MLflow server following the [Self-Hosting Guide](/docs/3.14.0/self-hosting.md), if you don't have one already.
...
MLflow automatically tracks token usage and cost for LangGraph. The token usage for each LLM call will be logged in each Trace/Span and the aggregated cost and time trend are displayed in the built-in dashboard. See the [Token Usage and Cost Tracking](/docs/3.14.0/genai/tracing/token-usage-cost.md) documentation for details on accessing this information programmatically.
...
By combining auto-tracing with the [manual tracing APIs](/docs/3.14.0/genai/tracing/app-instrumentation/manual-tracing.md), you can add child spans inside a node or tool, to get more detailed insights for the step.
...
![LangGraph Child Span](/docs/3.14.0/assets/images/langgraph-child-span-076b0cb599aeabce965b36602d5fda82.png)
...
![LangGraph Thread ID](/docs/3.14.0/assets/images/langgraph-thread-id-f180f62c969e783d2a312a5d69ccd2fd.png)

By navigating to the Session tab on the side bar, you can view all the traces in the session.

![LangGraph Session Page](/docs/3.14.0/assets/images/langgraph-session-page-c27d68787ae5bbefa08c8bb485273697.png)
...
When using LangGraph.js with OpenTelemetry-based tracing, you can combine the automatically generated traces with the [MLflow Tracing SDK](/docs/3.14.0/genai/tracing.md) (`@mlflow/core`) to add custom spans, set tags, and update trace metadata within the same trace.
...
For detailed instructions and examples, see [Combining the OpenTelemetry SDK and the MLflow Tracing SDK](/docs/3.14.0/genai/tracing/app-instrumentation/opentelemetry.md#combining-the-opentelemetry-sdk-and-the-mlflow-tracing-sdk).
