...
Start with [built-in judges](/docs/3.14.0/genai/eval-monitor/scorers/llm-judge/predefined.md#available-judges) for quick evaluation. As your needs evolve, build [custom LLM judges](/docs/3.14.0/genai/eval-monitor/scorers/llm-judge/custom-judges.md) for domain-specific criteria and create [custom code-based scorers](/docs/3.14.0/genai/eval-monitor/scorers/custom.md) for programmatic business logic.

| Approach                                                                                            | Level of customization | Use cases                                                                                                                                                             |
| --------------------------------------------------------------------------------------------------- | ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Built-in judges](/docs/3.14.0/genai/eval-monitor/scorers/llm-judge/predefined.md#available-judges) | Minimal                | Quickly try LLM evaluation with built-in judges such as `Correctness` and `RetrievalGroundedness`.                                                                    |
| [Guidelines judges](/docs/3.14.0/genai/eval-monitor/scorers/llm-judge/guidelines.md)                | Moderate               | A built-in judge that checks whether responses pass or fail custom natural-language rules, such as style or factuality guidelines.                                    |
| [Custom judges](/docs/3.14.0/genai/eval-monitor/scorers/llm-judge/custom-judges.md)                 | Full                   | Create fully customized LLM judges with detailed evaluation criteria and feedback optimization. Capable of returning numerical scores, categories, or boolean values. |
| [Code-based scorers](/docs/3.14.0/genai/eval-monitor/scorers/custom.md)                             | Full                   | Programmatic scorers that evaluate things like exact matching, format validation, and performance metrics.                                                            |

note

We'll refer to LLM judges and code-based scorers separately, but in the API, both LLM judges and code-based scorers are classified as types of scorers, such as in the functions [`list_scorers`](/docs/3.14.0/api_reference/python_api/mlflow.genai.html#mlflow.genai.scorers.list_scorers) and [`get_scorer`](/docs/3.14.0/api_reference/python_api/mlflow.genai.html#mlflow.genai.scorers.get_scorer)

## How judges work[​](#how-judges-work "Direct link to How judges work")

A judge receives a [Trace](/docs/3.14.0/genai/concepts/trace.md) from `evaluate()`. It then does the following:
...
3. Returns the quality assessment as [Feedback](/docs/3.14.0/genai/assessments/feedback.md) to attach to the `trace`
...
Judges use LLMs for evaluation. Use them directly with `mlflow.genai.evaluate()` or wrap them in [custom scorers](/docs/3.14.0/genai/eval-monitor/scorers/custom.md) for advanced scoring logic.
...
See the [complete list of built-in judges](/docs/3.14.0/genai/eval-monitor/scorers/llm-judge/predefined.md#available-judges) for details on each judge and their usage. You can further improve the judges' accuracy by [aligning them with human feedback](/docs/3.14.0/genai/eval-monitor/scorers/llm-judge/alignment.md).
...
See [Custom judges](/docs/3.14.0/genai/eval-monitor/scorers/llm-judge/custom-judges.md). Once you've created custom judges, you can further improve their accuracy by [aligning them with human feedback](/docs/3.14.0/genai/eval-monitor/scorers/llm-judge/alignment.md).
...
For a list of supported models, see [selecting judge models](/docs/3.14.0/genai/eval-monitor/scorers/llm-judge/custom-judges.md#selecting-judge-models).
...
 → Use [Built-in Judges](/docs/3.14.0/genai/eval-monitor/scorers/llm-judge/predefined.md) to get started.

> *I want to evaluate my application with a simple natural language criteria, such as "The response must be polite".*

 → Use [Guidelines-based Judges](/docs/3.14.0/genai/eval-monitor/scorers/llm-judge/guidelines.md).

> *I want to use more advanced prompt for evaluating my application.*

 → Use [Prompt-based Judges](/docs/3.14.0/genai/eval-monitor/scorers/llm-judge/custom-judges.md).

> *I want to dump the entire trace to the scorer and get detailed insights from it.*

 → Use [Trace-Based Judges](/docs/3.14.0/genai/eval-monitor/scorers/llm-judge/custom-judges.md#trace-based-judges).

> *I want to write my own code for evaluating my application. Other scorers don't fit my advanced needs.*

 → Use [Code-based Scorers](/docs/3.14.0/genai/eval-monitor/scorers/custom.md) to implement your own evaluation logic with Python.
...
See [Create custom code-based scorers](/docs/3.14.0/genai/eval-monitor/scorers/custom.md).
...
Start with generating [traces](/docs/3.14.0/genai/tracing.md) from a set of realistic input samples. If you already have production traces, that is even better.

2

#### Gather human feedback

Collect feedback from domain experts or users. MLflow provides [a UI and SDK](/docs/3.14.0/genai/assessments/feedback.md) for collecting feedback on traces.
...
To organize traces into error categories, use [Trace Tag](/docs/3.14.0/genai/tracing/attach-tags.md) to label and filter traces.

4

#### Translate failure modes into Judges

Define judges that check for the common failure modes. For example, if the answer is in an incorrect format, you may define an [LLM Judge](/docs/3.14.0/genai/eval-monitor/scorers.md#llms-as-judges) that checks if the format is correct. We recommend starting with a simple instruction and then iteratively refine it.

5

#### Align judges with human feedback.

LLM-as-a-Judge has natural biases. Relying on biased evaluation will lead to incorrect decision making. Therefore, it is important to refine the scorer to align with human feedback. You can manually iterate on prompts or instructions, or use the [Automatic Judge Alignment](/docs/3.14.0/genai/eval-monitor/scorers/llm-judge/alignment.md) feature of MLflow to optimize the instruction with a state-of-the-art algorithm powered by [DSPy](https://dspy.ai/).

Pro tip: Version Control Judges

As you iterate on the judge, version control becomes important. MLflow can track [Judge Versions](/docs/3.14.0/genai/eval-monitor/scorers/versioning.md) to help you maintain changes and share the improved judges with your team.

## Next Steps[​](#next-steps "Direct link to Next Steps")

### [LLM-based Judges](/docs/3.14.0/genai/eval-monitor/scorers.md#llms-as-judges)

[Get started with LLM judges for evaluating qualities.](/docs/3.14.0/genai/eval-monitor/scorers.md#llms-as-judges)

[Learn more →](/docs/3.14.0/genai/eval-monitor/scorers.md#llms-as-judges)

### [Evaluate Agents](/docs/3.14.0/genai/eval-monitor/running-evaluation/agents.md)

[Evaluate AI agents with specialized techniques and custom judges](/docs/3.14.0/genai/eval-monitor/running-evaluation/agents.md)

[Learn more →](/docs/3.14.0/genai/eval-monitor/running-evaluation/agents.md)

### [Collect Human Feedback](/docs/3.14.0/genai/assessments/feedback.md)

[Gather and manage human feedback to improve your evaluation accuracy](/docs/3.14.0/genai/assessments/feedback.md)

[Learn more →](/docs/3.14.0/genai/assessments/feedback.md)
