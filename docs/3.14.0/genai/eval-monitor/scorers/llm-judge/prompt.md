...
The `custom_prompt_judge` API is being phased out. We strongly recommend using the [make\_judge](/docs/3.14.0/api_reference/python_api/mlflow.genai.html#mlflow.genai.judges.make_judge) API instead, which provides:
...
See the [make\_judge documentation](/docs/3.14.0/genai/eval-monitor/scorers/llm-judge/custom-judges.md) for migration guidance.

The [custom\_prompt\_judge](/docs/3.14.0/api_reference/python_api/mlflow.genai.html#mlflow.genai.judges.custom_prompt_judge) API is designed to help you quickly and easily create LLM scorers when you need full control over the judge's prompt or need to return multiple output values beyond "pass" / "fail", for example, "great", "ok", "bad".
...
We recommend starting with [guidelines-based judges](/docs/3.14.0/genai/eval-monitor/scorers/llm-judge/guidelines.md) and only using prompt-based judges if you need more control or can't write your evaluation criteria as pass/fail guidelines. Guidelines-based judges have the distinct advantage of being easy to explain to business stakeholders and can often be directly written by domain experts.
...
MLflow uses raw prompt-based instructions for handling structured outputs to make the API generic to all LLM providers. This may not be strict enough to enforce structured outputs in all cases. If you see output parsing errors frequently, consider using [code-based custom scorers](/docs/3.14.0/genai/eval-monitor/scorers/custom.md) and invoke the specific structured output API for the LLM provider you are using to get more reliable results.

## Maintaining Your Prompt[​](#maintaining-your-prompt "Direct link to Maintaining Your Prompt")

Writing good prompts for LLM judges requires iterative testing and refinement. [MLflow Prompt Registry](/docs/3.14.0/genai/prompt-registry.md) is a great tool to help you manage and version control your prompts and share them with your team.
...
![Prompt Registry](/docs/3.14.0/images/mlflow-3/eval-monitor/scorers/prompt-registry.png)

## Selecting Judge Models[​](#selecting-judge-models "Direct link to Selecting Judge Models")

MLflow supports all major LLM providers, such as OpenAI, Anthropic, Google, xAI, and more. See [Supported Models](/docs/3.14.0/genai/eval-monitor/scorers/llm-judge/custom-judges/supported-models.md) for more details.
...
[Learn how to evaluate AI agents with specialized techniques and scorers](/docs/3.14.0/genai/eval-monitor/running-evaluation/agents.md)

[Learn more →](/docs/3.14.0/genai/eval-monitor/running-evaluation/agents.md)

### [Prompt Registry](/docs/3.14.0/genai/prompt-registry.md)

[Version control and manage your judge prompts with MLflow Prompt Registry](/docs/3.14.0/genai/prompt-registry.md)

[Learn more →](/docs/3.14.0/genai/prompt-registry.md)

### [Collect User Feedback](/docs/3.14.0/genai/assessments/feedback.md)

[Integrate user feedback to continuously improve your evaluation criteria and model performance](/docs/3.14.0/genai/assessments/feedback.md)
