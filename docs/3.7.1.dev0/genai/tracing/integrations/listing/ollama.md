...
![Ollama Tracing via autolog](/docs/3.7.1.dev0/assets/images/ollama-tracing-69a45b14d6b555fcbb9441a658445124.png)
...
MLflow >= 3.2.0 supports token usage tracking for local LLM endpoint served through OpenAI SDK. The token usage for each LLM call will be logged in the `mlflow.chat.tokenUsage` attribute. The total token usage throughout the trace will be available in the `token_usage` field of the trace info object.
