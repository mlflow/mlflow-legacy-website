...
In addition to the [Auto Tracing](/docs/3.6.1.dev0/genai/tracing/app-instrumentation/automatic.md) integrations, you can instrument your Python code using MLflow's manual tracing APIs. This is especially useful when you need to instrument your custom Python code.

tip

Looking for the Typescript guide? Checkout the [Typescript SDK](/docs/3.6.1.dev0/genai/tracing/app-instrumentation/typescript-sdk.md) documentation.

## Decorator[​](#decorator "Direct link to Decorator")

The [`mlflow.trace()`](/docs/3.6.1.dev0/api_reference/python_api/mlflow.html#mlflow.trace) decorator allows you to create a span for any function. This approach provides a simple yet effective way to add tracing to your code with minimal effort:
...
![Tracing Decorator](/docs/3.6.1.dev0/assets/images/trace-decorator-8ae22208121b562582947549f8b9a46e.png)
...
The [`mlflow.trace()`](/docs/3.6.1.dev0/api_reference/python_api/mlflow.html#mlflow.trace) decorator accepts following arguments to customize the span to be created:

* 🏷️ **`name` parameter** to override the span name from the default (the name of decorated function)
* 🎯 **`span_type` parameter** to set the type of span. Set either one of built-in [Span Types](/docs/3.6.1.dev0/genai/concepts/span.md#span-types) or a string.
...
Alternatively, you can update the span dynamically inside the function by using [`mlflow.get_current_active_span()`](/docs/3.6.1.dev0/api_reference/python_api/mlflow.html#mlflow.get_current_active_span) API.
...
Tags can be added to traces to provide additional metadata at the trace level. There are a few different ways to set tags on a trace. Please refer to the [how-to guide](/docs/3.6.1.dev0/genai/tracing/attach-tags.md) for the other methods.
...
![Trace Error](/docs/3.6.1.dev0/assets/images/trace-exception-d23766aa1c06b25b252c98d5d98cfef5.gif)

### Combining with Auto-Tracing[​](#combining-with-auto-tracing "Direct link to Combining with Auto-Tracing")

The `@mlflow.trace` decorator can be used in conjunction with auto tracing to combine auto-tracing with manually defined spans in a single cohesive and integrated trace. Learn more [here](/docs/3.6.1.dev0/genai/tracing/app-instrumentation/automatic.md#combining-manual-and-automatic-tracing).
...
Of course, we recommend using the [auto-tracing for OpenAI](/docs/3.6.1.dev0/genai/tracing/integrations/listing/openai.md) for examples like this, which does the same job but with one-liner code. The example below is for demonstration purposes.
...
Below is a slightly more complex example that uses the [`mlflow.start_span()`](/docs/3.6.1.dev0/api_reference/python_api/mlflow.html#mlflow.start_span) context manager in conjunction with both the decorator and auto-tracing for OpenAI.
...
![Multi threaded tracing](/docs/3.6.1.dev0/assets/images/tracing-multi-thread-acfb6f382ce45e030c95b7c0536749bf.png)
...
**[Combining with Auto-Tracing](/docs/3.6.1.dev0/genai/tracing/app-instrumentation/automatic.md#combining-manual-and-automatic-tracing)**: Mix automatic and manual tracing for optimal observability

**[Trace Concepts](/docs/3.6.1.dev0/genai/concepts/trace.md)**: Understand the structure and components of MLflow traces

**[Querying Traces](/docs/3.6.1.dev0/genai/tracing/search-traces.md)**: Programmatically search and analyze your traces
