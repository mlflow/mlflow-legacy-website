...
![CrewAI Tracing via autolog](/docs/3.6.1.dev0/assets/images/crewai-tracing-91ffc6888df3527199ebf2e25cf697d8.gif)

[CrewAI](https://www.crewai.com/) is an open-source framework for orchestrating role-playing, autonomous AI agent.

[MLflow Tracing](/docs/3.6.1.dev0/genai/tracing/integrations.md) provides automatic tracing capability for [CrewAI](https://www.crewai.com/), an open source framework for building multi-agent applications. By enabling auto tracing for CrewAI by calling the [`mlflow.crewai.autolog()`](/docs/3.6.1.dev0/api_reference/python_api/mlflow.crewai.html#mlflow.crewai.autolog) function, , MLflow will capture nested traces for CrewAI workflow execution and logged them to the active MLflow Experiment.
...
MLflow >= 3.5.0 supports token usage tracking for CrewAI. The token usage for each LLM call will be logged in the `mlflow.chat.tokenUsage` attribute. The total token usage throughout the trace will be available in the `token_usage` field of the trace info object.
