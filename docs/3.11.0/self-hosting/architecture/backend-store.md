...
Large model artifacts such as model weight files are stored in the [artifact store](/docs/3.11.0/self-hosting/architecture/artifact-store.md).
...
* Call [`mlflow.set_tracking_uri()`](/docs/3.11.0/api_reference/python_api/mlflow.html#mlflow.set_tracking_uri) in your code.
* If you are running a [Tracking Server](/docs/3.11.0/self-hosting/architecture/tracking-server.md), you can set the `--backend-store-uri` option when starting the server, like `mlflow server --backend-store-uri postgresql://...`

Continue to the next section for the supported format of tracking URLs. Also visit [this guidance](/docs/3.11.0/self-hosting/architecture/tracking-server.md) for how to set up the backend store properly for your workflow.
...
* HTTP server (specified as `https://my-server:5000`), which is a server hosting an [MLflow tracking server](/docs/3.11.0/self-hosting/architecture/tracking-server.md).
...
* **Model Registry Integration**: [Model Registry](/docs/3.11.0/ml/model-registry.md) functionality requires a database-backed store. See [this FAQ](/docs/3.11.0/ml/tracking.md#tracking-with-model-registry) for more information.
