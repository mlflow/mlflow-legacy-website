...
![Usage Modal](/docs/3.12.0/assets/images/usage-modal-d8b20f050157839fcbc443a603535b89.png)
...
For detailed information on passthrough APIs for each provider, see [Model Providers](/docs/3.12.0/genai/governance/ai-gateway/endpoints/model-providers.md).
...
When [workspaces](/docs/3.12.0/self-hosting/workspaces/getting-started.md) are enabled on your MLflow server, you can specify the target workspace for a request by including the `X-MLFLOW-WORKSPACE` header.
...
For more details on workspace configuration and management, see the [Workspaces documentation](/docs/3.12.0/self-hosting/workspaces/getting-started.md).
...
AI Gateway endpoints can be used as the backing LLM for MLflow's [LLM Judges](/docs/3.12.0/genai/eval-monitor/scorers.md#llms-as-judges). This allows you to run judge evaluations through the gateway, benefiting from centralized API key management and cost tracking.
...
For more details on creating and using LLM judges, see the [LLM Judges documentation](/docs/3.12.0/genai/eval-monitor/scorers.md#llms-as-judges).
