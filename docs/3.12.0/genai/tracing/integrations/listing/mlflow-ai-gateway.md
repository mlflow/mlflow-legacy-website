...
![MLflow AI Gateway Tracing](/docs/3.12.0/images/llms/tracing/basic-openai-trace.png)
...
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------- |
| **Server-side Tracing** | Gateway automatically logs all requests when [usage tracking](/docs/3.12.0/genai/governance/ai-gateway/usage-tracking.md) is enabled | Centralized tracing for all requests through the gateway   |
| **Client-side Tracing** | Use OpenAI SDK with MLflow autolog                                                                                                   | Combining LLM traces with your agent or application traces |

When both are used together with a `traceparent` header, the gateway creates a linked span under the agent's trace for end-to-end visibility. See [Distributed Tracing](/docs/3.12.0/genai/tracing/app-instrumentation/distributed-tracing.md#ai-gateway-integration) for details.
...
Create an endpoint in MLflow AI Gateway to route requests to your LLM provider. See the [AI Gateway Quickstart](/docs/3.12.0/genai/governance/ai-gateway/quickstart.md) for detailed setup instructions.
...
Since MLflow AI Gateway exposes an OpenAI-compatible API, you can use MLflow's [OpenAI automatic tracing](/docs/3.12.0/genai/tracing/integrations/listing/openai.md) integration to trace calls.
...
You can use MLflow's [Anthropic automatic tracing](/docs/3.12.0/genai/tracing/integrations/listing/anthropic.md) integration to trace calls through MLflow AI Gateway.
...
You can use MLflow's [Gemini automatic tracing](/docs/3.12.0/genai/tracing/integrations/listing/gemini.md) integration to trace calls through MLflow AI Gateway.
...
You can use MLflow's [LangChain automatic tracing](/docs/3.12.0/genai/tracing/integrations/listing/langchain.md) integration to trace calls through MLflow AI Gateway.
