...
The MLflow Tracking is an API and UI for logging parameters, code versions, metrics, and output files when running your machine learning code and for later visualizing the results. MLflow Tracking provides [Python](/docs/3.12.0/api_reference/python_api/index.html#) , [REST](/docs/3.12.0/api_reference/rest-api.html#) , [R](/docs/3.12.0/api_reference/R-api.html#), and [Java](/docs/3.12.0/api_reference/java_api/index.html#) APIs.

![](/docs/3.12.0/assets/images/tracking-metrics-ui-temp-ffc0da57b388076730e20207dbd7f9c4.png)
...
[MLflow Tracking Quickstart](/docs/3.12.0/ml/tracking/quickstart.md)

[A great place to start to learn the fundamentals of MLflow Tracking! Learn in 5 minutes how to log, register, and load a model for inference.](/docs/3.12.0/ml/tracking/quickstart.md)
...
An experiment groups together runs and models for a specific task. You can create an experiment using the CLI, API, or UI. The MLflow API and UI also let you create and search for experiments. See [Organizing Runs into Experiments](/docs/3.12.0/ml/tracking/tracking-api.md#experiment-organization) for more details on how to organize your runs into experiments.

## Tracking Runs[​](#start-logging "Direct link to Tracking Runs")

[MLflow Tracking APIs](/docs/3.12.0/ml/tracking/tracking-api.md) provide a set of functions to track your runs. For example, you can call [`mlflow.start_run()`](/docs/3.12.0/api_reference/python_api/mlflow.html#mlflow.start_run) to start a new run, then call [Logging Functions](/docs/3.12.0/ml/tracking/tracking-api.md) such as [`mlflow.log_param()`](/docs/3.12.0/api_reference/python_api/mlflow.html#mlflow.log_param) and [`mlflow.log_metric()`](/docs/3.12.0/api_reference/python_api/mlflow.html#mlflow.log_metric) to log parameters and metrics respectively. Please visit the [Tracking API documentation](/docs/3.12.0/ml/tracking/tracking-api.md) for more details about using these APIs.
...
Alternatively, [Auto-logging](/docs/3.12.0/ml/tracking/autolog.md) offers an ultra-quick setup for starting MLflow tracking. This powerful feature allows you to log metrics, parameters, and models without the need for explicit log statements - all you need to do is call [`mlflow.autolog()`](/docs/3.12.0/api_reference/python_api/mlflow.html#mlflow.autolog) before your training code. Auto-logging supports popular libraries such as [Scikit-learn](/docs/3.12.0/ml/tracking/autolog.md#autolog-sklearn), [XGBoost](/docs/3.12.0/ml/tracking/autolog.md#autolog-xgboost), [PyTorch](/docs/3.12.0/ml/tracking/autolog.md#autolog-pytorch), [Keras](/docs/3.12.0/ml/tracking/autolog.md#autolog-keras), [Spark](/docs/3.12.0/ml/tracking/autolog.md#autolog-spark), and more. See [Automatic Logging Documentation](/docs/3.12.0/ml/tracking/autolog.md) for supported libraries and how to use auto-logging APIs with each of them.
...
MLflow 3 introduces powerful model search capabilities through [`mlflow.search_logged_models()`](/docs/3.12.0/api_reference/python_api/mlflow.html#mlflow.search_logged_models). This API allows you to find specific models across your experiments based on performance metrics, parameters, and model attributes using SQL-like syntax.
...
For comprehensive examples and advanced search patterns, see the [Search Logged Models Guide](/docs/3.12.0/ml/search/search-models.md).

### Querying Runs Programmatically[​](#tracking_query_api "Direct link to Querying Runs Programmatically")

You can also access all of the functions in the Tracking UI programmatically with [MlflowClient](/docs/3.12.0/api_reference/python_api/mlflow.client.html#mlflow.client.MlflowClient).
...
Use [`mlflow.search_logged_models()`](/docs/3.12.0/api_reference/python_api/mlflow.html#mlflow.search_logged_models) to search and rank model checkpoints based on their performance metrics:
...
MLflow offers the ability to track datasets that are associated with model training events. These metadata associated with the Dataset can be stored through the use of the [`mlflow.log_input()`](/docs/3.12.0/api_reference/python_api/mlflow.html#mlflow.log_input) API. To learn more, please visit the [MLflow data documentation](/docs/3.12.0/ml/dataset.md) to see the features available in this API.
...
![MLflow UI Experiment view page models tab](/docs/3.12.0/assets/images/tracking-models-ui-0f88d40c517e103cdead462aab12781a.png)
...
#### [MLflow Tracking APIs](/docs/3.12.0/ml/tracking/tracking-api.md)[​](#mlflow-tracking-apis "Direct link to mlflow-tracking-apis")
...
#### [Backend Store](/docs/3.12.0/self-hosting/architecture/backend-store.md)[​](#backend-store "Direct link to backend-store")
...
#### [Artifact Store](/docs/3.12.0/self-hosting/architecture/artifact-store.md)[​](#artifact-stores "Direct link to artifact-stores")
...
If the model is registered in the [MLflow Model Registry](/docs/3.12.0/ml/model-registry.md), you can also refer to the model through a model URI of format: `models:/<model-name>/<model-version>`, see [MLflow Model Registry](/docs/3.12.0/ml/model-registry.md) for details.

#### [MLflow Tracking Server](/docs/3.12.0/self-hosting/architecture/tracking-server.md) (Optional)[​](#tracking_server "Direct link to tracking_server")

MLflow Tracking Server is a stand-alone HTTP server that provides REST APIs for accessing backend and/or artifact store. Tracking server also offers flexibility to configure what data to serve, govern access control, versioning, and etc. Read [MLflow Tracking Server documentation](/docs/3.12.0/self-hosting.md) for more details.
...
![](/docs/3.12.0/assets/images/tracking-setup-overview-3d8cfd511355d9379328d69573763331.png)
...
![](/docs/3.12.0/assets/images/tracking-setup-local-server-cd51180e89bfd0a18c52f5b33e0f188d.png)
...
Yes, MLflow supports launching multiple runs in parallel e.g. multi processing / threading. See [Launching Multiple Runs in One Program](/docs/3.12.0/ml/tracking/tracking-api.md#parallel-execution-strategies) for more details.
...
* [Organize runs into experiments](/docs/3.12.0/ml/tracking/tracking-api.md#experiment-organization) - Experiments are logical containers for your runs. You can create an experiment using the CLI, API, or UI.
* [Create child runs](/docs/3.12.0/ml/tracking/tracking-api.md#hierarchical-runs-with-parent-child-relationships) - You can create child runs under a single parent run to group them together. For example, you can create a child run for each fold in a cross-validation experiment.
* [Add tags to runs](/docs/3.12.0/ml/tracking/tracking-api.md#smart-tagging-for-organization) - You can associate arbitrary tags with each run, which allows you to filter and search runs based on tags.
...
1. Set up artifacts configuration such as credentials and endpoints, just like you would for the MLflow Tracking Server. See [configure artifact storage](/docs/3.12.0/self-hosting/architecture/artifact-store.md#artifacts-store-supported-storages) for more details.
...
A system tag `mlflow.note.content` can be used to add descriptive note about this run. While the other [system tags](/docs/3.12.0/ml/tracking/tracking-api.md#system-tags-reference) are set automatically, this tag is **not set by default** and users can override it to include additional information about the run. The content will be displayed on the run's page under the Notes section.
