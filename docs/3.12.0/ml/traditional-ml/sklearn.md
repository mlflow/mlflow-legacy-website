...
Running locally? MLflow stores experiments in the current directory by default. For team collaboration or remote tracking, **[set up a tracking server](/docs/3.12.0/ml/tracking/tutorials/remote-server.md)**.
...
The `nested=True` parameter creates child runs for each trial under the parent run, enabling hierarchical organization of hyperparameter tuning experiments. Learn more about **[hierarchical runs](/docs/3.12.0/ml/tracking/tracking-api.md#hierarchical-runs-with-parent-child-relationships)**.
