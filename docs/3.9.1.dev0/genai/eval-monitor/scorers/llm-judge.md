...
The fastest way to create LLM judges is through the **Judge Builder UI** - no code required. Navigate to your experiment's **Judges** tab to create and test judges visually. See the [Template-based Scorers](/docs/3.9.1.dev0/genai/eval-monitor/scorers/llm-judge/make-judge.md) page for details. Requires MLflow >= 3.9.0.

## Approaches for Creating LLM Scorers[​](#approaches-for-creating-llm-scorers "Direct link to Approaches for Creating LLM Scorers")
...
[Template-based Scorers](/docs/3.9.1.dev0/genai/eval-monitor/scorers/llm-judge/make-judge.md)

[Simplicity: ★★★☆☆    Control: ★★★★☆](/docs/3.9.1.dev0/genai/eval-monitor/scorers/llm-judge/make-judge.md)

***

[* **Best for**: Creating custom LLM judges with natural language instructions. Includes built-in versioning and human feedback alignment.* **How it works**: Define evaluation criteria using template variables (`inputs`, `outputs`, `expectations`) in plain English. MLflow can automatically optimize the judge template with human feedback for improved accuracy.* **Requires**: MLflow >= 3.4.0](/docs/3.9.1.dev0/genai/eval-monitor/scorers/llm-judge/make-judge.md)

[Guidelines-based Scorers](/docs/3.9.1.dev0/genai/eval-monitor/scorers/llm-judge/guidelines.md)

[Simplicity: ★★★★☆    Control: ★★★☆☆](/docs/3.9.1.dev0/genai/eval-monitor/scorers/llm-judge/guidelines.md)

***

[* **Best for**: Evaluations based on a simple set of natural language criteria, framed as pass/fail conditions. Ideal for checking compliance with rules, style guides, or information inclusion/exclusion.* **How it works**: You provide a set of plain-language rules that refer to specific inputs or outputs from your app, for example 'The response must be polite'. An LLM then determines if the guideline passes or fails and provides a rationale. You can think of it as a simpler version of prompt-based judge.](/docs/3.9.1.dev0/genai/eval-monitor/scorers/llm-judge/guidelines.md)

[Predefined Scorers](/docs/3.9.1.dev0/genai/eval-monitor/scorers/llm-judge/predefined.md)

[Simplicity: ★★★★★    Control: ★☆☆☆☆](/docs/3.9.1.dev0/genai/eval-monitor/scorers/llm-judge/predefined.md)

***

[* **Best for**: Quickly trying MLflow's LLM evaluation capabilities with a few lines of code.* **How it works**: Select from a list of built-in classes such as Correctness, RetrievalGroundedness, etc. MLflow constructs inputs for the judge using predefined prompt templates.](/docs/3.9.1.dev0/genai/eval-monitor/scorers/llm-judge/predefined.md)
...
[AI Gateway](/docs/3.9.1.dev0/genai/governance/ai-gateway.md) endpoints are the recommended way to configure judge models, especially when creating judges from the UI. Benefits include:
