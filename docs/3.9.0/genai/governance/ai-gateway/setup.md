...
How to rotate the encryption passphrase (KEK)

The encryption passphrase (KEK - Key Encryption Key) is the master secret used to encrypt all stored API keys in the database. You should rotate this periodically for security best practices.
...
**Critical:** You must update BOTH `MLFLOW_CRYPTO_KEK_PASSPHRASE` and `MLFLOW_CRYPTO_KEK_VERSION` environment variables. Failure to update both will cause decryption failures when the server attempts to read stored API keys.

## Accessing the Gateway[​](#accessing-the-gateway "Direct link to Accessing the Gateway")

Once your MLflow server is running, navigate to the AI Gateway section at `http://localhost:5000/#/gateway`.

![Gateway Overview](/docs/3.9.0/assets/images/gateway-overview-2bbf3cf41c12806962300767a53094c0.png)
...
## Managing API Keys[​](#managing-api-keys "Direct link to Managing API Keys")
...
![API Keys Page](/docs/3.9.0/assets/images/api-keys-page-120794235927669caacd55978043270f.png)
...
When you need to rotate credentials from your LLM provider (OpenAI, Anthropic, etc.) for security purposes:
...
Endpoints define how requests are routed to AI models. For advanced routing features like traffic splitting and fallbacks, see the [Advanced Routing](/docs/3.9.0/genai/governance/ai-gateway/routing.md) guide.

### Basic Endpoint Setup[​](#basic-endpoint-setup "Direct link to Basic Endpoint Setup")
...
![Create Endpoint](/docs/3.9.0/assets/images/create-endpoint-4b5244302954cd8e676b9f929168a048.png)
