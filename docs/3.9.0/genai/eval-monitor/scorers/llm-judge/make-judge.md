...
* **SDK**: The `make_judge` API requires **MLflow >= 3.4.0**. For earlier versions, use the deprecated [custom\_prompt\_judge](/docs/3.9.0/api_reference/python_api/mlflow.genai.html#mlflow.genai.judges.custom_prompt_judge) instead.
...
![Judges Tab](/docs/3.9.0/images/mlflow-3/eval-monitor/scorers/judges-tab.png)
...
![Test Judge Output](/docs/3.9.0/images/mlflow-3/eval-monitor/scorers/test-judge-output.png)
...
6. **Schedule automatic evaluation** (optional):
   ...
   * **Filter string**: Only evaluate traces matching this filter ([syntax](/docs/3.9.0/genai/tracing/search-traces.md))

7. Click **Create judge** to save your new LLM judge

The [make\_judge](/docs/3.9.0/api_reference/python_api/mlflow.genai.html#mlflow.genai.judges.make_judge) API is the recommended way to create custom LLM judges programmatically.
...
To evaluate a batch of responses and track results in MLflow, use the [mlflow.genai.evaluate](/docs/3.9.0/api_reference/python_api/mlflow.genai.html#mlflow.genai.evaluate) function.
...
![Make Judge Example](/docs/3.9.0/images/mlflow-3/eval-monitor/scorers/make-judge-scorer.png)
...
| `trace`        | Trace is a special template variable which uses [agent-as-a-judge](/docs/3.9.0/genai/eval-monitor/scorers/llm-judge/agentic-overview.md). The judge has access to all parts of the trace. |
...
* **[AI Gateway](/docs/3.9.0/genai/governance/ai-gateway.md) endpoints** (Recommended for UI) - AI Gateway centralizes LLM access, which provides key benefits for evaluation workflows:
...
* **SDK**: Use the `feedback_value_type` argument in [make\_judge](/docs/3.9.0/api_reference/python_api/mlflow.genai.html#mlflow.genai.judges.make_judge). Supported types include `bool`, `int`, `float`, `str`, `Literal` for categorical outcomes, `dict[str, <primitive>]`, and `list[<primitive>]`.

## Versioning Scorers[​](#versioning-scorers "Direct link to Versioning Scorers")

To get reliable scorers, iterative refinement is necessary. [Tracking scorer versions](/docs/3.9.0/genai/eval-monitor/scorers/versioning.md) helps you maintain and iterate on your scorers without losing track of changes.
...
LLMs have biases and errors. Relying on biased evaluation will lead to incorrect decision making. Use [Automatic Judge Alignment](/docs/3.9.0/genai/eval-monitor/scorers/llm-judge/alignment.md) to optimize instructions using human feedback, powered by the state-of-the-art [SIMBA algorithm from DSPy](https://dspy.ai/api/optimizers/SIMBA/). For rapid iteration during development, the experimental [**MemAlign**](/docs/3.9.0/genai/eval-monitor/scorers/llm-judge/memalign.md) optimizer achieves competitive quality with up to 100× faster and 10× cheaper alignment using just a handful of examples.

## Next Steps[​](#next-steps "Direct link to Next Steps")

### [Evaluation Quickstart](/docs/3.9.0/genai/eval-monitor/quickstart.md)

[Get started with MLflow's evaluation framework.](/docs/3.9.0/genai/eval-monitor/quickstart.md)

[Start evaluating →](/docs/3.9.0/genai/eval-monitor/quickstart.md)

### [Collect Human Feedback](/docs/3.9.0/genai/assessments/feedback.md)

[Learn how to collect human feedback for evaluation.](/docs/3.9.0/genai/assessments/feedback.md)

[Collect feedback →](/docs/3.9.0/genai/assessments/feedback.md)

### [Aligning Judges with Human Feedback](/docs/3.9.0/genai/eval-monitor/scorers/llm-judge/alignment.md)

[Learn how to align your scorer with human feedback.](/docs/3.9.0/genai/eval-monitor/scorers/llm-judge/alignment.md)

[Learn alignment →](/docs/3.9.0/genai/eval-monitor/scorers/llm-judge/alignment.md)
