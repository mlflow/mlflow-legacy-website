...
* **Sharing Results**: The tracking server also serves [Tracking UI](/docs/3.9.0/ml/tracking.md#tracking_ui) endpoint, where team members can easily explore each other's results.
...
This section describes how to configure the tracking server for some common use cases. The section requires you to have a basic knowledge about the tracking server architecture, please visit [Architecture Overview](/docs/3.9.0/self-hosting/architecture/overview.md) if you are not familiar with it yet.
...
For this reason, the client needs direct access to the artifact store. For instructions on setting up these credentials, see [Artifact Stores documentation](/docs/3.9.0/self-hosting/architecture/artifact-store.md#artifacts-stores-manage-access).
...
For detailed configuration options, see [Security Configuration](/docs/3.9.0/self-hosting/security/network.md).
...
For notebook integration and UI embedding options, see [Network Security](/docs/3.9.0/self-hosting/security/network.md) configuration.
