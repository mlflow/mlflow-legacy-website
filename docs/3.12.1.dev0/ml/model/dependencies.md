...
[MLflow Model](/docs/3.12.1.dev0/ml/model.md) is a standard format that packages a machine learning model with its dependencies and other metadata. Building a model with its dependencies allows for reproducibility and portability across a variety of platforms and tools.

When you create an MLflow model using the [MLflow Tracking APIs](/docs/3.12.1.dev0/ml/tracking.md), for instance, [`mlflow.pytorch.log_model()`](/docs/3.12.1.dev0/api_reference/python_api/mlflow.pytorch.html#mlflow.pytorch.log_model), MLflow automatically infers the required dependencies for the model flavor you're using and records them as a part of Model metadata. Then, when you serve the model for prediction, MLflow automatically installs the dependencies to the environment. Therefore, you normally won't need to worry about managing dependencies in MLflow Model.
...
One tip for improving MLflow's dependency inference accuracy is to add an `input_example` when saving your model. This enables MLflow to perform a model prediction before saving the model, thereby capturing the dependencies used during the prediction. Please refer to [Model Input Example](/docs/3.12.1.dev0/ml/model/signatures.md) for additional, detailed usage of this parameter.
...
* [Adding Extra Dependencies to an MLflow Model](#adding-extra-dependencies-to-an-mlflow-model)
* [Defining All Dependencies by Yourself](#defining-all-dependencies-by-yourself)
* [Saving Extra Code dependencies with an MLflow Model - Automatic inference](#saving-code-dependencies)
* [Saving Extra Code with an MLflow Model - Manual Declaration](#saving-extra-code-with-an-mlflow-model---manual-declaration)
* [Validating Environment for Prediction](#validating-environment-for-prediction)
* [Troubleshooting](#model-dependencies-troubleshooting)

## How MLflow Records Model Dependencies[​](#how-mlflow-records-dependencies "Direct link to How MLflow Records Model Dependencies")
...
Model dependencies are defined by the following files (For other files, please refer to the guidance provided in the section discussing [Storage Format](/docs/3.12.1.dev0/ml/model.md#storage-format)):
...
### Locking Model Dependencies with MLFLOW\_LOCK\_MODEL\_DEPENDENCIES[​](#locking-model-dependencies-with-mlflow_lock_model_dependencies "Direct link to Locking Model Dependencies with MLFLOW_LOCK_MODEL_DEPENDENCIES")
...
## Using uv for Dependency Management[​](#uv-dependency-management "Direct link to Using uv for Dependency Management")

note

Experimental: These parameters may change or be removed in a future release without warning.

[uv](https://docs.astral.sh/uv/) is a fast Python package manager that supports lockfile-based dependency resolution. When your project uses uv, MLflow can read directly from your `uv.lock` file to capture exact, reproducible dependencies when logging a model. This avoids relying solely on import-based inference and ensures that every transitive dependency is pinned to the version you tested with.

note

**Difference from `MLFLOW_LOCK_MODEL_DEPENDENCIES`**

MLflow provides two separate mechanisms for capturing pinned dependencies:

* **[`MLFLOW_LOCK_MODEL_DEPENDENCIES`](#locking-model-dependencies-with-mlflow_lock_model_dependencies)**: Triggers on-the-fly resolution and pinning at model logging time from the inferred requirements. It does not require a pre-existing `uv.lock` file, but because dependencies are re-resolved from the inferred requirement spec, the resulting versions may differ from the exact packages currently installed in your environment.

* **Using uv for Dependency Management** (this section): Reads dependencies directly from an existing `uv.lock` file in your uv-managed project. This approach captures the exact versions you locked and tested with, rather than re-resolving at log time.

Use this section's approach if your project already uses uv as its package manager and you have a `uv.lock` file. Use `MLFLOW_LOCK_MODEL_DEPENDENCIES` if you want automatic transitive dependency locking without managing a uv project.

### Limitations[​](#limitations "Direct link to Limitations")

* **uv workspaces are not supported.** If your project uses a [uv workspace](https://docs.astral.sh/uv/concepts/projects/workspaces/), MLflow will not correctly handle workspace members. Each workspace member must be treated as a standalone uv project for MLflow uv integration to work.

### How It Works[​](#how-it-works "Direct link to How It Works")

When you log a model, MLflow checks for a `uv.lock` and `pyproject.toml` in the current working directory. If both files exist and uv (version 0.6.10 or later) is installed, MLflow runs `uv export` to produce a pinned requirements list from the lockfile. This list replaces the standard import-based dependency inference.

MLflow also copies the uv project files (`uv.lock` and `pyproject.toml`) and, if present, the `.python-version` file into the model artifact directory. These files enable exact environment restoration with `uv sync` when the model is loaded later.

This works with all MLflow model flavors. For example, logging a scikit-learn model in a uv-managed project:
...
By default, uv project detection is enabled. MLflow looks for `uv.lock` and `pyproject.toml` in the current working directory when you log a model with any flavor, including [`mlflow.sklearn.log_model()`](/docs/3.12.1.dev0/api_reference/python_api/mlflow.sklearn.html#mlflow.sklearn.log_model), [`mlflow.pytorch.log_model()`](/docs/3.12.1.dev0/api_reference/python_api/mlflow.pytorch.html#mlflow.pytorch.log_model), [`mlflow.transformers.log_model()`](/docs/3.12.1.dev0/api_reference/python_api/mlflow.transformers.html#mlflow.transformers.log_model), [`mlflow.pyfunc.log_model()`](/docs/3.12.1.dev0/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.log_model), and others. All flavors that call `infer_pip_requirements` benefit from uv auto-detection.
...
You can also control whether uv project files (`uv.lock`, `pyproject.toml`, and `.python-version`) are saved as model artifacts with `MLFLOW_LOG_UV_FILES`:
...
### Specifying a uv Project Path[​](#specifying-a-uv-project-path "Direct link to Specifying a uv Project Path")

If your uv project is not in the current working directory (common in monorepos or non-standard layouts), use the `uv_project_path` parameter to point to the correct directory:
...
MLflow will look for `uv.lock` and `pyproject.toml` in the specified directory instead of the current working directory.

### Including Dependency Groups[​](#including-dependency-groups "Direct link to Including Dependency Groups")

uv supports [dependency groups](https://docs.astral.sh/uv/concepts/dependencies/#dependency-groups) for organizing related packages (such as a `ml` group for model-specific libraries). Use the `uv_groups` parameter to include one or more groups when exporting requirements:
...
This maps to `uv export --group ml --group data`. The group dependencies are included in addition to the project's default dependencies.

### Including Optional Extras[​](#including-optional-extras "Direct link to Including Optional Extras")

uv supports [optional extras](https://docs.astral.sh/uv/concepts/dependencies/#optional-dependencies) defined in `pyproject.toml` (the standard `[project.optional-dependencies]` table). Use the `uv_extras` parameter to include one or more extras:
...
This maps to `uv export --extra gpu --extra serving`.

For a complete, runnable example with a uv project (including `pyproject.toml`, `uv.lock`, and a script demonstrating auto-detection, explicit paths, groups, and extras), see the [full example code](https://github.com/mlflow/mlflow/tree/master/examples/uv-dependency-management).

tip

The `uv_project_path`, `uv_groups`, and `uv_extras` parameters are available on [`mlflow.pyfunc.log_model()`](/docs/3.12.1.dev0/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.log_model) and [`mlflow.pyfunc.save_model()`](/docs/3.12.1.dev0/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.save_model). For other flavors (sklearn, pytorch, transformers, etc.), uv auto-detection works automatically when `uv.lock` and `pyproject.toml` exist in the current working directory.

warning

uv version 0.6.10 or later must be installed for these features to work. If uv is not available, MLflow falls back to standard import-based dependency inference and logs a warning.

## Adding Extra Dependencies to an MLflow Model[​](#adding-extra-dependencies-to-an-mlflow-model "Direct link to Adding Extra Dependencies to an MLflow Model")

MLflow infers dependencies required for the model flavor library, but your model may depend on other libraries e.g. data preprocessing. In this case, you can add extra dependencies to the model by specifying the **extra\_pip\_requirements** param when logging the model. For example,
...
Once you log the model with dependencies, it is advisable to test it in a sandbox environment to avoid any dependency issues when deploying the model to production. Since MLflow 2.10.0, you can use the [`mlflow.models.predict()`](/docs/3.12.1.dev0/api_reference/python_api/mlflow.models.html#mlflow.models.predict) API to quickly test your model in a virtual environment. Please refer to [Validating Environment for Prediction](#validating-environment-for-prediction) for more details.
...
Alternatively, you can also define all dependencies from scratch rather than adding extra ones. To do so, specify **pip\_requirements** when logging the model. For example,
...
With `infer_code_paths` set to `True`, the dependency of `map_iris_types` will be analyzed, its source declaration detected as originating in the `custom_code.py` module, and the code reference within `custom_code.py` will be stored along with the model artifact. Note that defining the external code dependency by using the `code_paths` argument (discussed in the next section) is not needed.
...
### Restrictions with `infer_code_paths`[​](#restrictions-with-infer_code_paths "Direct link to restrictions-with-infer_code_paths")
...
## Saving Extra Code with an MLflow Model - Manual Declaration[​](#saving-extra-code-with-an-mlflow-model---manual-declaration "Direct link to Saving Extra Code with an MLflow Model - Manual Declaration")

MLflow also supports saving your custom Python code as dependencies to the model. This is particularly useful when you want to deploy your custom modules that are required for prediction with the model. To do so, specify **code\_paths** when logging the model. For example, if you have the following file structure in your project:
...
### Use of `code_paths` Option for a Custom Library[​](#use-of-code_paths-option-for-a-custom-library "Direct link to use-of-code_paths-option-for-a-custom-library")

To include custom libraries that are not publicly available on PyPI when logging your model, the `code_paths` argument can be used. This option allows you to upload `.whl` files or other dependencies alongside your model, ensuring all required libraries are available during serving.

warning

The following example demonstrates a quick method for including custom libraries for development purposes. This approach is not recommended for production environments. For production usage, upload libraries to a custom PyPI server or a cloud storage to ensure reliable and secure access.
...
This limitation is due to how MLflow saves and loads the specified files and directories. When it copies the specified files or directories in `code/` target, it does **not** preserve the relative paths that they were originally residing within. For instance, in the above example, MLflow will copy `utils.py` to `code/utils.py`, not `code/src/utils.py`. As a result, it has to be imported as `from utils import my_func`, instead of `from src.utils import my_func`. However, this may not be pleasant, as the import path is different from the original training script.
...
### Limitation of `code_paths` in loading multiple models with the same module name but different implementations[​](#limitation-of-code_paths-in-loading-multiple-models-with-the-same-module-name-but-different-implementations "Direct link to limitation-of-code_paths-in-loading-multiple-models-with-the-same-module-name-but-different-implementations")
...
This way you can log the model with `code_paths=["core"]` to include the required modules for prediction, while excluding the helper modules that are only used for development.

## Validating Environment for Prediction[​](#validating-environment-for-prediction "Direct link to Validating Environment for Prediction")

Validating your model before deployment is a critical step to ensure production readiness. MLflow provides a few ways to test your model locally, either in a virtual environment or a Docker container. If you find any dependency issues during validation, please follow the guidance in [How to fix dependency errors when serving my model?](#how-to-fix-dependency-errors-in-model)
...
You can use MLflow Models **predict** API via Python or CLI to make test predictions with your model. This will load your model from the model URI, create a virtual environment with the model dependencies (defined in MLflow Model), and run offline predictions with the model. Please refer to [`mlflow.models.predict()`](/docs/3.12.1.dev0/api_reference/python_api/mlflow.models.html#mlflow.models.predict) or the [CLI reference](/docs/3.12.1.dev0/api_reference/cli.html#mlflow-models) for more detailed usage for the predict API.
...
Using the [`mlflow.models.predict()`](/docs/3.12.1.dev0/api_reference/python_api/mlflow.models.html#mlflow.models.predict) API is convenient for testing your model and inference environment quickly. However, it may not be a perfect simulation of the serving because it does not start the online inference server. That said, it's a great way to test whether your prediction inputs are correctly formatted.

Formatting is subject to the types supported by the `predict()` method of your logged model. If the model was logged with a signature, the input data should be viewable from the MLflow UI or via [`mlflow.models.get_model_info()`](/docs/3.12.1.dev0/api_reference/python_api/mlflow.models.html#mlflow.models.get_model_info), which has the field `signature`.
...
## Troubleshooting[​](#model-dependencies-troubleshooting "Direct link to Troubleshooting")

### How to Fix Dependency Errors when Serving my Model[​](#how-to-fix-dependency-errors-in-model "Direct link to How to Fix Dependency Errors when Serving my Model")
...
Now that you know the missing dependencies, you can create a new model version with the correct dependencies. However, creating a new model for trying new dependencies might be a bit tedious, particularly because you may need to iterate multiple times to find the correct solution. Instead, you can use the [`mlflow.models.predict()`](/docs/3.12.1.dev0/api_reference/python_api/mlflow.models.html#mlflow.models.predict) API to test your change without actually needing to re-log the model repeatedly while troubleshooting the installation errors.
...
Once you find the correct dependencies, you can update the logged model's dependencies using [`mlflow.models.update_model_requirements()`](/docs/3.12.1.dev0/api_reference/python_api/mlflow.models.html#mlflow.models.update_model_requirements) API.
...
### How to enable installing pre-releases when using `mlflow.models.predict`[​](#how-to-enable-installing-pre-releases-when-using-mlflowmodelspredict "Direct link to how-to-enable-installing-pre-releases-when-using-mlflowmodelspredict")
...
### How to Migrate Anaconda Dependency for License Change[​](#how-to-migrate-anaconda-dependency-for-license-change "Direct link to How to Migrate Anaconda Dependency for License Change")
...
For more information on the `log_model()` API, see the MLflow documentation for the model flavor you are working with, for example, [`mlflow.sklearn.log_model()`](/docs/3.12.1.dev0/api_reference/python_api/mlflow.sklearn.html#mlflow.sklearn.log_model).
