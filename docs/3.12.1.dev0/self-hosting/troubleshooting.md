...
## MLflow UI/SDK is slow[​](#mlflow-uisdk-is-slow "Direct link to MLflow UI/SDK is slow")
...
For connecting to different databases such as PostgreSQL, see [backend store documentation](/docs/3.12.1.dev0/self-hosting/architecture/backend-store.md).
...
## The database or storage is full. Deleting runs/models does not work.[​](#the-database-or-storage-is-full-deleting-runsmodels-does-not-work "Direct link to The database or storage is full. Deleting runs/models does not work.")
...
## Should I use the same MLflow version for the client and the server?[​](#should-i-use-the-same-mlflow-version-for-the-client-and-the-server "Direct link to Should I use the same MLflow version for the client and the server?")
