...
## Supported Content Types[​](#supported-content-types "Direct link to Supported Content Types")
...
Images are represented as [`mlflow.types.chat.ImageContentPart()`](/docs/3.12.1.dev0/api_reference/python_api/mlflow.types.html#mlflow.types.chat.ImageContentPart) objects with type `image_url`. The `image_url.url` field accepts:
...
Audio is represented as [`mlflow.types.chat.AudioContentPart()`](/docs/3.12.1.dev0/api_reference/python_api/mlflow.types.html#mlflow.types.chat.AudioContentPart) objects with type `input_audio`. The `input_audio.data` field contains base64-encoded audio data, and `input_audio.format` specifies the codec (`wav` or `mp3`).

Audio content is rendered with an inline audio player in the **Chat** tab of the trace viewer.

## Automatic Base64 Extraction[​](#automatic-base64-extraction "Direct link to Automatic Base64 Extraction")
...
When using [auto-instrumentation](/docs/3.12.1.dev0/genai/tracing/app-instrumentation/automatic.md), multimodal content is captured automatically. MLflow normalizes provider-specific formats into the standard schema described above, and base64 content is extracted into attachments.
...
### OpenAI — Image (URL)[​](#openai--image-url "Direct link to OpenAI — Image (URL)")
...
### OpenAI — Image (Base64)[​](#openai--image-base64 "Direct link to OpenAI — Image (Base64)")
...
### OpenAI — Audio[​](#openai--audio "Direct link to OpenAI — Audio")
...
### OpenAI — Image Generation[​](#openai--image-generation "Direct link to OpenAI — Image Generation")
...
### Anthropic — Image[​](#anthropic--image "Direct link to Anthropic — Image")
...
You can attach multimodal content to traces manually using [`mlflow.start_span()`](/docs/3.12.1.dev0/api_reference/python_api/mlflow.html#mlflow.start_span) and `set_inputs()` / `set_outputs()`. Structure the messages with content parts lists, following the same format used by the OpenAI API:
...
![Image content rendered inline in the trace viewer](/docs/3.12.1.dev0/images/llms/tracing/trace-image-content.png)

![Audio content rendered with an inline player in the trace viewer](/docs/3.12.1.dev0/images/llms/tracing/trace-audio-player.png)
...
## Trace Attachments[​](#trace-attachments "Direct link to Trace Attachments")
...
### Supported Content Types[​](#supported-content-types-1 "Direct link to Supported Content Types")
...
### Using Attachments in Spans[​](#using-attachments-in-spans "Direct link to Using Attachments in Spans")
...
### Multiple Attachments[​](#multiple-attachments "Direct link to Multiple Attachments")
...
### When to Use Explicit Attachments[​](#when-to-use-explicit-attachments "Direct link to When to Use Explicit Attachments")
