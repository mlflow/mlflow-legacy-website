...
* Training a linear regression model with [MLflow Tracking](/docs/3.10.0/ml/tracking.md).
* Conducting hyper-parameter tuning to find the best model.
* Packaging the model weights and dependencies as an [MLflow Model](/docs/3.10.0/ml/model.md).
* Testing model serving locally with [mlserver](https://mlserver.readthedocs.io/en/latest/) using the [mlflow models serve](/docs/3.10.0/api_reference/cli.html#mlflow-models-serve) command.
...
While KServe enables highly scalable and production-ready model serving, deploying your model there might require some effort. MLflow simplifies the process of deploying models to a Kubernetes cluster with KServe and MLServer. Additionally, it offers seamless **end-to-end model management** as a single place to manage the entire ML lifecycle. This includes [experiment tracking](/docs/3.10.0/ml/tracking.md), [model packaging](/docs/3.10.0/ml/model.md), [versioning](/docs/3.10.0/ml/model-registry.md), [evaluation](/docs/3.10.0/ml/evaluation.md), and [deployment](/docs/3.10.0/ml/deployment.md), which we will cover in this tutorial.
...
For the sake of convenience, we use the [`mlflow.sklearn.autolog()`](/docs/3.10.0/api_reference/python_api/mlflow.sklearn.html#mlflow.sklearn.autolog) function. This function allows MLflow to automatically log the appropriate set of model parameters and metrics during training. To learn more about the auto-logging feature or how to log manually instead, see the [MLflow Tracking documentation](/docs/3.10.0/ml/tracking.md).
...
![](/docs/3.10.0/assets/images/tracking-ui-default-620cdc67da31f91f653210afe78de3b8.png)
...
![](/docs/3.10.0/assets/images/hyper-parameter-tuning-ui-88db40c21c63fb307e84629607985a73.png)
...
To learn more about hyperparameter tuning with MLflow, please refer to [Hyperparameter Tuning with MLflow and Optuna](/docs/3.10.0/ml/traditional-ml/tutorials/hyperparameter-tuning.md).
...
Since we are using autologging, MLflow automatically logs the [Model](/docs/3.10.0/ml/model.md) for each run. This process conveniently packages the model weight and dependencies in a ready-to-deploy format.

note

In practice, it is also recommended to use [MLflow Model Registry](/docs/3.10.0/ml/model-registry.md) for registering and managing your models.
...
If you opt for manual logging, you will need to log the model explicitly using the [`mlflow.sklearn.log_model`](/docs/3.10.0/api_reference/python_api/mlflow.sklearn.html#mlflow.sklearn.log_model) function, as shown below:
...
For more information about the request format and response formats, refer to the [Inference Server Specification](/docs/3.10.0/ml/deployment/deploy-model-locally.md#local-inference-server-spec).
...
* [MLflow Tracking](/docs/3.10.0/ml/tracking.md) - Explore more about MLflow Tracking and various ways to manage experiments and models, such as team collaboration.
* [MLflow Model Registry](/docs/3.10.0/ml/model-registry.md) - Discover more about MLflow Model Registry for managing model versions and stages in a centralized model store.
* [MLflow Deployment](/docs/3.10.0/ml/deployment.md) - Learn more about MLflow deployment and different deployment targets.
