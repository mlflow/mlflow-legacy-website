...
* **Sharing Results**: The tracking server also serves [Tracking UI](/docs/3.15.0/ml/tracking.md#tracking_ui) endpoint, where team members can easily explore each other's results.
...
Once the tracking server is started, connect your local clients by setting the `MLFLOW_TRACKING_URI` environment variable to the server's URI, along with its scheme and port (for example, `http://10.0.0.1:5000`) or call [`mlflow.set_tracking_uri()`](/docs/3.15.0/api_reference/python_api/mlflow.html#mlflow.set_tracking_uri).

MLflow APIs like [`mlflow.start_run()`](/docs/3.15.0/api_reference/python_api/mlflow.html#mlflow.start_run), [`mlflow.log_param()`](/docs/3.15.0/api_reference/python_api/mlflow.html#mlflow.log_param), [`mlflow.start_trace()`](/docs/3.15.0/api_reference/python_api/mlflow.html#mlflow.start_trace) make API requests to your remote tracking server and log the data.
...
This section describes how to configure the tracking server for some common use cases. The section requires you to have a basic knowledge about the tracking server architecture, please visit [Architecture Overview](/docs/3.15.0/self-hosting/architecture/overview.md) if you are not familiar with it yet.
...
| Isolate experiments, models, and prompts | `--enable-workspaces`                                   | [Getting Started with Workspaces](/docs/3.15.0/self-hosting/workspaces/getting-started.md) |
...
### Trace Archival[​](#trace-archival "Direct link to Trace Archival")

MLflow can archive older trace span payloads from the tracking database to a configured artifact location while keeping the traces readable in the UI and APIs.

note

**Requirements**

Trace archival requires a tracking store that supports server-owned archival. The archival `location` must be a real artifact repository URI. The scheduler only archives finalized traces; in-progress traces are skipped.
...
Then start the server with that configuration, either through the environment or with the `--trace-archival-config` flag:
...
* `max_traces_per_pass` limits the amount of work in a single pass. If you omit it, the pass is unbounded and MLflow archives every eligible trace it can process in that run.
* `long_retention_allowlist` contains experiment IDs that are allowed to keep traces longer than the broader server or workspace policy.
...
2. Workspace-level `trace_archival_config` overrides those defaults for all traces in the workspace.
3. Experiment-level settings can further reduce retention for a specific experiment.
4. If an experiment requests a longer retention period than the broader server or workspace policy, that longer retention is only honored when the experiment's ID appears in `long_retention_allowlist`.

The archival budget remains server-owned even when workspaces are enabled. `max_traces_per_pass` applies to the entire scheduler pass, not to each workspace independently. On each pass MLflow shuffles the workspace order, then processes one workspace at a time. That means a workspace visited earlier in the shuffled order can consume some or all of the remaining pass budget before later workspaces are considered. Within a workspace, `archive-now` candidates are still prioritized before regular retention-based archival.

For example, if the global default is 30 days, a workspace is configured for 90 days, and experiment `123` is configured for 14 days, traces for experiment `123` are retained for 14 days. If experiment `123` is instead configured for 180 days, its traces are retained for 90 days unless `123` is added to `long_retention_allowlist`, in which case they are retained for 180 days.

Troubleshooting:

* If trace archival never runs, confirm that `MLFLOW_TRACE_ARCHIVAL_CONFIG` points to a valid YAML file and that `trace_archival.enabled` is `true`.
* Changes to `MLFLOW_TRACE_ARCHIVAL_CONFIG` are picked up without restarting MLflow. The server refreshes the config automatically with a short cache window (about 5 seconds) and applies updated settings on later scheduler checks. If a config refresh fails, MLflow continues using the last valid config until the file is fixed.
* If a workspace or experiment does not show an effective retention, check whether archival is disabled, misconfigured, or missing an archival location.
* If a longer experiment retention is ignored, add that experiment ID to `long_retention_allowlist` in the server config.

See [Archive Traces](/docs/3.15.0/genai/tracing/observe-with-traces/archive-traces.md) for the end-user workflow and archival failure / retry behavior, and [Getting Started with Workspaces](/docs/3.15.0/self-hosting/workspaces/getting-started.md) for workspace-level examples.
...
If you are using remote storage, you have to configure the credentials for the server to access the artifacts. Be aware of that The MLflow artifact proxied access service enables users to have an *assumed role of access to all artifacts* that are accessible to the Tracking Server. Refer [Manage Access](/docs/3.15.0/self-hosting/architecture/artifact-store.md#artifacts-stores-manage-access) for further details.
...
For this reason, the client needs direct access to the artifact store. For instructions on setting up these credentials, see [Artifact Stores documentation](/docs/3.15.0/self-hosting/architecture/artifact-store.md#artifacts-stores-manage-access).
...
#### Optionally using a Tracking Server instance exclusively for artifact handling[​](#tracking-server-artifacts-only "Direct link to Optionally using a Tracking Server instance exclusively for artifact handling")
...
MLflow 3.5.0+ includes built-in security middleware to protect against DNS rebinding and CORS attacks. When using `--host 0.0.0.0`, configure the `--allowed-hosts` option to specify which domains can access your server. See [Security Configuration](/docs/3.15.0/self-hosting/security/network.md) for details.
...
For detailed configuration options, see [Security Configuration](/docs/3.15.0/self-hosting/security/network.md).
...
For notebook integration and UI embedding options, see [Network Security](/docs/3.15.0/self-hosting/security/network.md) configuration.
...
### Example: Restricting to MLflow Artifacts[​](#example-restricting-to-mlflow-artifacts "Direct link to Example: Restricting to MLflow Artifacts")
...
## Full List of Command Line Options[​](#full-list-of-command-line-options "Direct link to Full List of Command Line Options")
