...
* [Tracing QuickStart](/docs/3.15.0/genai/tracing/quickstart.md)
* [LLM Evaluation Quickstart](/docs/3.15.0/genai/eval-monitor/quickstart.md)
* [Prompt Management Quickstart](/docs/3.15.0/genai/prompt-registry.md#getting-started)
* [Model Training Quickstart](/docs/3.15.0/ml/tracking/quickstart.md)

tip

For production deployments or custom backend configurations, see [Backend Store](/docs/3.15.0/self-hosting/architecture/backend-store.md) documentation.
...
MLflow provides an official Helm chart for deploying on Kubernetes. See the [Kubernetes Helm Deployment guide](/docs/3.15.0/self-hosting/kubernetes-helm.md) for step-by-step installation instructions, configuration options, and production examples.
...
To learn more about the architecture and available backend options, see [Architecture](/docs/3.15.0/self-hosting/architecture/overview.md).

## Workspaces[​](#workspaces "Direct link to Workspaces")

MLflow supports [workspaces](/docs/3.15.0/self-hosting/workspaces.md) to organize experiments, registered models, prompts, and artifacts on a shared MLflow instance. Workspaces add logical separation and workspace-level permissions so teams can collaborate without running separate servers. Workspaces are opt-in and require a SQL database backend.

## Access Control & Security[​](#access-control--security "Direct link to Access Control & Security")

MLflow support [username/password login](/docs/3.15.0/self-hosting/security/basic-http-auth.md) via basic HTTP authentication, [SSO (Single Sign-On)](/docs/3.15.0/self-hosting/security/sso.md), and [custom authentication plugins](/docs/3.15.0/self-hosting/security/custom.md).

MLflow also provides built-in [network protection](/docs/3.15.0/self-hosting/security/network.md) middleware to protect your tracking server from network exposure.
...
See [Troubleshooting & FAQs](/docs/3.15.0/self-hosting/troubleshooting.md) for more information.
...
Refer to the [Network Security Guide](/docs/3.15.0/self-hosting/security/network.md) for detailed configuration options.
