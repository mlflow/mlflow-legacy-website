...
Custom code-based [scorers](/docs/3.15.0/genai/eval-monitor/scorers.md) offer the ultimate flexibility to define precisely how your LLM application or AI agent's quality is measured. You can define evaluation metrics tailored to your specific business use case, whether based on simple heuristics, advanced logic, or programmatic evaluations.

Use custom scorers for the following scenarios:

1. Defining a custom heuristic or code-based evaluation metric.
2. Customizing how the data from your app's trace is mapped to MLflow's [research-backed LLM judges](/docs/3.15.0/genai/eval-monitor/scorers/llm-judge/predefined.md#available-judges).
3. Using your own LLM for evaluation.
4. Any other use cases where you need more flexibility and control than provided by [custom LLM judges](/docs/3.15.0/genai/eval-monitor/scorers/llm-judge/custom-judges.md).

For a tutorial with many examples, see [Code-based scorer examples](/docs/3.15.0/genai/eval-monitor/scorers/custom/code-examples.md).

Custom scorers are for offline evaluation only

Custom code-based scorers (both the [`@scorer` decorator](#define-scorers-with-the-scorer-decorator) and the [`Scorer` class](#define-scorers-with-the-scorer-class)) work with [`mlflow.genai.evaluate()`](/docs/3.15.0/api_reference/python_api/mlflow.genai.html#mlflow.genai.evaluate) for offline evaluation. They are **not supported** for [automatic evaluation](/docs/3.15.0/genai/eval-monitor/automatic-evaluations.md) (production monitoring), which only supports [LLM judges](/docs/3.15.0/genai/eval-monitor/scorers.md#llms-as-judges) such as [built-in judges](/docs/3.15.0/genai/eval-monitor/scorers/llm-judge/predefined.md), [`make_judge()`](/docs/3.15.0/genai/eval-monitor/scorers/llm-judge/custom-judges/create-custom-judge.md), and [Guidelines](/docs/3.15.0/genai/eval-monitor/scorers/llm-judge/guidelines.md).

## How custom scorers work[​](#how-custom-scorers-work "Direct link to How custom scorers work")

Custom scorers are written in Python and give you full control to evaluate any data from your app's traces. After you define a custom scorer, you can use it with [`mlflow.genai.evaluate()`](/docs/3.15.0/api_reference/python_api/mlflow.genai.html#mlflow.genai.evaluate) just like a [built-in LLM Judge](/docs/3.15.0/genai/eval-monitor/scorers/llm-judge/predefined.md#available-judges).
...
![Code-based Scorers](/docs/3.15.0/images/mlflow-3/eval-monitor/scorers/code-scorers-results.png)

The code snippet below defines these custom scorers and uses it with [`mlflow.genai.evaluate()`](/docs/3.15.0/api_reference/python_api/mlflow.genai.html#mlflow.genai.evaluate):
...
1. [The `@scorer` decorator](/docs/3.15.0/genai/eval-monitor/scorers/custom.md#define-scorers-with-the-scorer-decorator) is used to define the scorer.
2. [The input](/docs/3.15.0/genai/eval-monitor/scorers/custom.md#inputs) to this scorer is the full trace, giving it access to the AI app's inputs, intermediate spans, and outputs.
3. Scorer logic can be fully custom. You can call LLMs or other scorers.
4. [The output](/docs/3.15.0/genai/eval-monitor/scorers/custom.md#outputs) of this scorer is a rich `Feedback` object with values and explanations.
5. [The metric name](/docs/3.15.0/genai/eval-monitor/scorers/custom.md#metric-naming-behavior) is `llm_response_time_good`, matching the scorer function name.
...
Most code-based scorers should be defined using the [`@scorer` decorator](/docs/3.15.0/api_reference/python_api/mlflow.genai.html#mlflow.genai.scorers.scorer). Below is the signature for such scorers, illustrating possible inputs and outputs.
...
Scorers receive the complete [MLflow trace](/docs/3.15.0/genai/concepts/trace.md) containing all spans, attributes, and outputs. As a convenience, MLflow also extracts commonly needed data and passes it as named arguments. All input arguments are optional, so declare only what your scorer needs:
...
* `trace`: The complete [MLflow trace](/docs/3.15.0/genai/concepts/trace.md) with all spans, allowing analysis of intermediate steps, latency, tool usage, and more. The trace is passed to the custom scorer as an instantiated [`mlflow.entities.Trace` class](/docs/3.15.0/api_reference/python_api/mlflow.entities.html#mlflow.entities.Trace) class.

When running [mlflow.genai.evaluate()](/docs/3.15.0/api_reference/python_api/mlflow.genai.html#mlflow.genai.evaluate), the `inputs`, `outputs`, and `expectations` parameters can be specified in the `data` argument, or parsed from the trace.
...
| [`Feedback`](/docs/3.15.0/api_reference/python_api/mlflow.entities.html#mlflow.entities.Feedback)      | Value + rationale | Detailed assessment     |
| List\[[Feedback](/docs/3.15.0/api_reference/python_api/mlflow.entities.html#mlflow.entities.Feedback)] | Multiple metrics  | Multi-aspect evaluation |
...
Return a [`Feedback`](/docs/3.15.0/api_reference/python_api/mlflow.entities.html#mlflow.entities.Feedback) object or list of `Feedback` objects for detailed assessments with scores, rationales, and metadata.
...
When you define scorers using either the [`@scorer`](/docs/3.15.0/api_reference/python_api/mlflow.genai.html#mlflow.genai.scorers.scorer) decorator or the [`Scorer` class](#define-scorers-with-the-scorer-class), the metric names in the [evaluation runs](/docs/3.15.0/genai/eval-monitor.md#running-an-evaluation) created by evaluation and monitoring follow simple rules:
...
See [examples of naming behavior](/docs/3.15.0/genai/eval-monitor/scorers/custom/code-examples.md) in the tutorial.
...
For custom error handling or to provide specific error messages, catch exceptions and return a [`Feedback`](/docs/3.15.0/api_reference/python_api/mlflow.entities.html#mlflow.entities.Feedback) with `None` value and error details:
...
* [`AssessmentError()`](/docs/3.15.0/api_reference/python_api/mlflow.entities.html#mlflow.entities.Feedback): For structured error reporting with error codes
...
The [`@scorer` decorator](#define-scorers-with-the-scorer-decorator) described above is simple and generally recommended, but when it is insufficient, you can instead use the [`Scorer`](/docs/3.15.0/api_reference/python_api/mlflow.genai.html#mlflow.genai.Scorer) base class. Class-based definitions allow for more complex scorers, especially scorers that require state. The [`Scorer`](/docs/3.15.0/api_reference/python_api/mlflow.genai.html#mlflow.genai.Scorer) class is a [Pydantic object](https://docs.pydantic.dev/latest/concepts/models/), so you can define additional fields and use them in the `__call__` method.
...
### [Code-based scorer examples](/docs/3.15.0/genai/eval-monitor/scorers/custom/code-examples.md)

[See many examples of code-based scorers](/docs/3.15.0/genai/eval-monitor/scorers/custom/code-examples.md)

[Learn more →](/docs/3.15.0/genai/eval-monitor/scorers/custom/code-examples.md)

### [Develop code-based scorers](/docs/3.15.0/genai/eval-monitor/scorers/custom/tutorial.md)

[Step through the development workflow for custom scorers](/docs/3.15.0/genai/eval-monitor/scorers/custom/tutorial.md)

[Learn more →](/docs/3.15.0/genai/eval-monitor/scorers/custom/tutorial.md)

### [Evaluate during development](/docs/3.15.0/genai/eval-monitor.md)

[Understand how mlflow.genai.evaluate() uses your scorers](/docs/3.15.0/genai/eval-monitor.md)

[Learn more →](/docs/3.15.0/genai/eval-monitor.md)
