...
## Building Chains with LangChain[​](#building-chains-with-langchain "Direct link to Building Chains with LangChain")
...
The `langchain` model flavor enables logging of [LangChain models](https://github.com/hwchase17/langchain) in MLflow format via the [`mlflow.langchain.save_model()`](/docs/3.11.1.dev0/api_reference/python_api/mlflow.langchain.html#mlflow.langchain.save_model) and [`mlflow.langchain.log_model()`](/docs/3.11.1.dev0/api_reference/python_api/mlflow.langchain.html#mlflow.langchain.log_model) functions. Use of these functions also adds the `python_function` flavor to the MLflow Models that they produce, allowing the model to be interpreted as a generic Python function for inference via [`mlflow.pyfunc.load_model()`](/docs/3.11.1.dev0/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.load_model).

You can also use the [`mlflow.langchain.load_model()`](/docs/3.11.1.dev0/api_reference/python_api/mlflow.langchain.html#mlflow.langchain.load_model) function to load a saved or logged MLflow Model with the `langchain` flavor as a dictionary of the model's attributes.
...
### Logging a LangChain Agent with MLflow[​](#logging-a-langchain-agent-with-mlflow "Direct link to Logging a LangChain Agent with MLflow")

#### What is an Agent?[​](#what-is-an-agent "Direct link to What is an Agent?")

Agents in LangChain leverage language models to dynamically determine and execute a sequence of actions, contrasting with the hardcoded sequences in chains. To learn more about Agents and see additional examples within LangChain, you can [read the LangChain docs on Agents](https://python.langchain.com/docs/how_to/#agents).

#### Key Components of Agents[​](#key-components-of-agents "Direct link to Key Components of Agents")
...
##### Additional Agent Runtimes[​](#additional-agent-runtimes "Direct link to Additional Agent Runtimes")
...
#### An Example of Logging an LangChain Agent[​](#an-example-of-logging-an-langchain-agent "Direct link to An Example of Logging an LangChain Agent")
...
### Using `predict_stream` for Streaming Outputs[​](#using-predict_stream-for-streaming-outputs "Direct link to using-predict_stream-for-streaming-outputs")
...
## Enhanced Management of RetrievalQA Chains with MLflow[​](#enhanced-management-of-retrievalqa-chains-with-mlflow "Direct link to Enhanced Management of RetrievalQA Chains with MLflow")
...
### Key Insights into RetrievalQA Chains[​](#key-insights-into-retrievalqa-chains "Direct link to Key Insights into RetrievalQA Chains")
...
### Detailed Overview of the RAG Process[​](#detailed-overview-of-the-rag-process "Direct link to Detailed Overview of the RAG Process")
...
### Clarifying Vector Database Management with MLflow[​](#clarifying-vector-database-management-with-mlflow "Direct link to Clarifying Vector Database Management with MLflow")
...
### Key MLflow Components and VectorDB Logging[​](#key-mlflow-components-and-vectordb-logging "Direct link to Key MLflow Components and VectorDB Logging")

1. **persist\_dir**: The directory where the retriever object, including the vector database, is stored.

2. **loader\_fn**: The function for loading the retriever object from its storage location.
...
### An Example of logging a LangChain RetrievalQA Chain[​](#an-example-of-logging-a-langchain-retrievalqa-chain "Direct link to An Example of logging a LangChain RetrievalQA Chain")
...
#### Purpose of Logging Individual Retrievers[​](#purpose-of-logging-individual-retrievers "Direct link to Purpose of Logging Individual Retrievers")
...
#### Requirements for Logging Retrievers in MLflow[​](#requirements-for-logging-retrievers-in-mlflow "Direct link to Requirements for Logging Retrievers in MLflow")

* **persist\_dir**: Specifies where the retriever object is stored.
* **loader\_fn**: Details the function used to load the retriever object from its storage location.
* These requirements align with those for logging RetrievalQA chains, ensuring consistency in the process.

#### An example of logging a LangChain Retriever[​](#an-example-of-logging-a-langchain-retriever "Direct link to An example of logging a LangChain Retriever")
...
Please refer to the [MLflow Langchain Autologging ](/docs/3.11.1.dev0/genai/flavors/langchain/autologging.md)documentation for more details on how to enable autologging for Langchain models.
