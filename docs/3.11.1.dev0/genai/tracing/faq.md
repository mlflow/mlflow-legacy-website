...
## Getting Started and Basic Usage[​](#getting-started-and-basic-usage "Direct link to Getting Started and Basic Usage")

### Q: How do I start using MLflow Tracing?[​](#q-how-do-i-start-using-mlflow-tracing "Direct link to Q: How do I start using MLflow Tracing?")
...
MLflow provides automatic tracing (autolog) for 40+ popular libraries. See the complete list at [Automatic Tracing Integrations](/docs/3.11.1.dev0/genai/tracing/integrations.md).
...
2. You call [`mlflow.search_traces()`](/docs/3.11.1.dev0/api_reference/python_api/mlflow.html#mlflow.search_traces)
...
You can customize what appears in the Request and Response columns of the trace list using [`mlflow.update_current_trace()`](/docs/3.11.1.dev0/api_reference/python_api/mlflow.html#mlflow.update_current_trace):
...
### Q: How to optimize the trace size in production?[​](#q-how-to-optimize-the-trace-size-in-production "Direct link to Q: How to optimize the trace size in production?")
...
To register a custom hooks, use the [`mlflow.tracing.configure`](/docs/3.11.1.dev0/api_reference/python_api/mlflow.tracing.html#mlflow.tracing.configure) API. For example, the following code filters out the document contents from the retriever span output to reduce the trace size:
...
Refer to the [Redacting Sensitive Data Safe](/docs/3.11.1.dev0/genai/tracing/observe-with-traces/masking.md) guide for more details about the hook API and examples.
...
### Q: My traces are not appearing in the MLflow UI. What could be wrong?[​](#q-my-traces-are-not-appearing-in-the-mlflow-ui-what-could-be-wrong "Direct link to Q: My traces are not appearing in the MLflow UI. What could be wrong?")
...
To **disable** tracing, [`mlflow.tracing.disable()`](/docs/3.11.1.dev0/api_reference/python_api/mlflow.tracing.html#mlflow.tracing.disable) API will cease the collection of trace data from within MLflow and will not log any data to the MLflow Tracking service regarding traces.

To **enable** tracing (if it had been temporarily disabled), [`mlflow.tracing.enable()`](/docs/3.11.1.dev0/api_reference/python_api/mlflow.tracing.html#mlflow.tracing.enable) API will re-enable tracing functionality for instrumented models that are invoked.
...
### Q: How do I associate traces with MLflow runs?[​](#q-how-do-i-associate-traces-with-mlflow-runs "Direct link to Q: How do I associate traces with MLflow runs?")
...
You can delete traces using the [`mlflow.client.MlflowClient.delete_traces()`](/docs/3.11.1.dev0/api_reference/python_api/mlflow.client.html#mlflow.client.MlflowClient.delete_traces) method:
...
Read more about [trace deletion](/docs/3.11.1.dev0/genai/tracing/observe-with-traces/delete-traces.md).
...
## Integration and Compatibility[​](#integration-and-compatibility "Direct link to Integration and Compatibility")

### Q: Is MLflow Tracing compatible with other observability tools?[​](#q-is-mlflow-tracing-compatible-with-other-observability-tools "Direct link to Q: Is MLflow Tracing compatible with other observability tools?")
...
For production monitoring, see [Production Tracing](/docs/3.11.1.dev0/genai/tracing/prod-tracing.md) for integration patterns.
...
Yes, MLflow provides comprehensive manual tracing capabilities. Please refer to the [Manual Tracing](/docs/3.11.1.dev0/genai/tracing/app-instrumentation/manual-tracing.md) guide for detailed information on creating traces and spans manually using decorators, context managers, and low-level APIs.
...
**Documentation**: Start with the [MLflow Tracing documentation](/docs/3.11.1.dev0/genai/tracing.md)
...
*For additional questions or issues not covered here, please check the [MLflow documentation](/docs/3.11.1.dev0/genai/tracing.md) or reach out to the community.*
