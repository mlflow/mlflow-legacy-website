...
The [`mlflow.pyfunc`](/docs/3.11.1.dev0/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc) module provides [save\_model()](/docs/3.11.1.dev0/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.save_model) and

[log\_model()](/docs/3.11.1.dev0/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.log_model) utilities for creating MLflow Models with the `python_function` flavor that contain user-specified code and *artifact* (file) dependencies.
...
There are two ways to define a PythonModel: Subclassing [`mlflow.pyfunc.PythonModel()`](/docs/3.11.1.dev0/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.PythonModel) or defining a callable. This guide provides a complete walkthrough on how to define and use a custom PythonModel.
...
The [`mlflow.pyfunc`](/docs/3.11.1.dev0/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc) module provides a [generic PythonModel class](/docs/3.11.1.dev0/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.PythonModel) that can be used to define your own customized model. By subclassing it, the model can be seamlessly integrated with other MLflow components.
...
* **load\_context** Implement the load\_context method if the model requires additional context to be loaded. For more details, refer to [load\_context()](/docs/3.11.1.dev0/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.PythonModel.load_context).

tip

Starting from MLflow 2.20.0, the `context` parameter can be removed from `predict` and `predict_stream` functions if it is not used. e.g. `def predict(self, model_input, params)` is a valid predict function signature.
...
An alternative way to log a PythonModel is to define a callable that **takes a single argument** and returns a prediction. This callable can be logged as a PythonModel by passing it to `mlflow.pyfunc.log_model`.

tip

Starting from MLflow 2.20.0, you can use the `@pyfunc` decorator on the callable to enable data validation on the input based on the type hints. Check [type hint usage in PythonModel](#type-hint-usage-in-pythonmodel) for more details.
...
Use the pyfunc module to log a custom model with [`mlflow.pyfunc.log_model()`](/docs/3.11.1.dev0/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.log_model).
...
Use [`mlflow.models.predict()`](/docs/3.11.1.dev0/api_reference/python_api/mlflow.models.html#mlflow.models.predict) API to validate the model dependencies and input data before deploy it. Check [MLflow Model Validation](/docs/3.11.1.dev0/ml/model.md#validate-models-before-deployment) for more details.
...
The final step to use your model in production is to deploy it. Follow [MLflow Model Deployment](/docs/3.11.1.dev0/ml/model.md#deployment_plugin) guide to deploy the model.
...
**Type hints used within the PythonModel's input signature must be of type `list[...]`** because PythonModel's predict function expects batch input data. The following type hints are supported as the element type of `list[...]`:
...
By subclassing [`mlflow.pyfunc.PythonModel()`](/docs/3.11.1.dev0/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.PythonModel), you can get data validation based on the type hints for free. The data validation works for both a PythonModel instance and a loaded PyFunc model.
...
### Pydantic Model type hints data conversion[​](#pydantic-model-type-hints-data-conversion "Direct link to Pydantic Model type hints data conversion")

For Pydantic model type hints, the input data can be either a Pydantic object or a dictionary that matches the schema of the Pydantic model. MLflow automatically converts the provided data to the type hint object before passing it to the predict function. If comparing to the example in the last section, `[{"role": "system", "content": "Hello"}]` is converted to `[Message(role="system", content="Hello")]` within the predict function.
...
Do not pass `signature` parameter explicitly when logging a PythonModel with type hints. If you pass the `signature` parameter, MLflow still uses the inferred signature based on the type hints, and raises a warning if they don't match.

The table below illustrates how type hints map to given schemas in model signatures:

| Type hint                 | Inferred schema                                                                         |
| ------------------------- | --------------------------------------------------------------------------------------- |
...
Pydantic objects cannot be used in the `infer_signature` function. To use pydantic objects as model inputs, you must define the type hints as the pydantic model in the PythonModel's predict function signature.

### Input example together with type hints during model logging[​](#input-example-together-with-type-hints-during-model-logging "Direct link to Input example together with type hints during model logging")

When logging a PythonModel, it is recommended to provide an input example that matches the type hints defined in the model. The input example is used to validate the type hints and check if the `predict` function works as expected.
...
When querying a serving endpoint hosting a PythonModel with type hints, you **must pass the input data with** `inputs` **key in the request body**. The example below demonstrates how to serve the model locally and query it:
...
MLflow also supports using the following type hints in PythonModel, but they are not used for data validation or schema inference, and a valid model signature or input\_example needs to be provided during model logging.
...
### TypeFromExample type hint usage[​](#typefromexample-type-hint-usage "Direct link to TypeFromExample type hint usage")

MLflow provides a special type hint, `TypeFromExample`, which helps convert the input data to match the type of your input example during PyFunc prediction. This is useful if you don't want to explicitly define a type hint for the model input but still want the data to conform to the input example type during prediction. **To use this feature, a valid input example must be provided during model logging.** The input example must be one of the following types, as the `predict` function expects batch input data:
...
If neither type hints nor `TypeFromExample` are used, MLflow's schema enforcement will default to converting the input data into a pandas DataFrame. This may not be ideal if the model expects the same type as the input example. It is strongly recommended to use supported type hints to avoid this conversion and enable data validation based on the specified type hints.
