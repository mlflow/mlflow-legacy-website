...
When you log a model, MLflow checks for a `uv.lock` and `pyproject.toml` in the current working directory. If both files exist and uv (version 0.5.0 or later) is installed, MLflow runs `uv export` to produce a pinned requirements list from the lockfile. This list replaces the standard import-based dependency inference.
...
uv version 0.5.0 or later must be installed for these features to work. If uv is not available, MLflow falls back to standard import-based dependency inference and logs a warning.
