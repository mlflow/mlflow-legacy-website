...
The `mlflow.data` module is a comprehensive solution for dataset management throughout the ML model development workflow. It enables you to track, version, and manage datasets used in training, validation, and evaluation, providing complete lineage from raw data to model predictions.

## Why Dataset Tracking Matters[​](#why-dataset-tracking-matters "Direct link to Why Dataset Tracking Matters")
...
* [`PandasDataset`](/docs/3.11.1.dev0/api_reference/python_api/mlflow.data.html#mlflow.data.pandas_dataset.PandasDataset) - For Pandas DataFrames
* [`SparkDataset`](/docs/3.11.1.dev0/api_reference/python_api/mlflow.data.html#mlflow.data.spark_dataset.SparkDataset) - For Apache Spark DataFrames
* [`NumpyDataset`](/docs/3.11.1.dev0/api_reference/python_api/mlflow.data.html#mlflow.data.numpy_dataset.NumpyDataset) - For NumPy arrays
* [`PolarsDataset`](/docs/3.11.1.dev0/api_reference/python_api/mlflow.data.html#mlflow.data.polars_dataset.PolarsDataset) - For Polars DataFrames
* [`HuggingFaceDataset`](/docs/3.11.1.dev0/api_reference/python_api/mlflow.data.html#mlflow.data.huggingface_dataset.HuggingFaceDataset) - For Hugging Face datasets
* [`TensorFlowDataset`](/docs/3.11.1.dev0/api_reference/python_api/mlflow.data.html#mlflow.data.tensorflow_dataset.TensorFlowDataset) - For TensorFlow datasets
* [`MetaDataset`](/docs/3.11.1.dev0/api_reference/python_api/mlflow.data.html#mlflow.data.meta_dataset.MetaDataset) - For metadata-only datasets (no actual data storage)
...
### DatasetSource[​](#datasetsource "Direct link to DatasetSource")
...
The `DatasetSource` can be retrieved using the [`mlflow.data.get_source()`](/docs/3.11.1.dev0/api_reference/python_api/mlflow.data.html#mlflow.data.get_source) API, which accepts instances of `Dataset`, `DatasetEntity`, or `DatasetInput`.
...
source = HTTPDatasetSource(url="https://ai.stanford.edu/~amaas/data/sentiment/aclImdb_v1.tar.gz")
...
![Dataset in MLflow UI](/docs/3.11.1.dev0/assets/images/dataset-mlflow-ui-9c475f4c9e28b0c438dafb3b3e8c9db8.png)
...
## Integration with MLflow Evaluate[​](#integration-with-mlflow-evaluate "Direct link to Integration with MLflow Evaluate")
    ...
    with mlflow.start_run(run_name="Model_Comparison"):
        for dataset_name, dataset in datasets_dict.items():
            with mlflow.start_run(run_name=f"Eval_{dataset_name}", nested=True):
                ...
                mlflow.log_params({"dataset_name": dataset_name, "dataset_size": len(dataset.df)})
...
![Dataset Evaluation in MLflow UI](/docs/3.11.1.dev0/assets/images/dataset-evaluate-0d449786c6575ea6b14818f6a3a92855.png)
...
## Advanced Dataset Management[​](#advanced-dataset-management "Direct link to Advanced Dataset Management")
...
def monitor_dataset_quality(dataset, reference_dataset=None):
    ...
    with mlflow.start_run(run_name="Data_Quality_Check"):
        ...
        if reference_dataset is not None:
            ref_data = (
                reference_dataset.df if hasattr(reference_dataset, "df") else reference_dataset
            )
            ...
            for col in numeric_cols:
                if col in ref_data.columns:
                    ...
                    drift_metrics.update({
                        f"{col}_mean_drift": mean_diff,
                        f"{col}_std_drift": std_diff,
                    })
...
class DatasetTracker:
    ...
    def track_dataset(self, data, stage, source=None, name=None, **metadata):
        ...
        with mlflow.start_run(run_name=f"Pipeline_{stage}"):
            ...
            # Log stage metadata
            mlflow.log_params({"pipeline_stage": stage, "dataset_name": dataset_name, **metadata})
...
cleaned_dataset = tracker.track_dataset(cleaned_data, "cleaned", source="cleaned_data.csv")
features_dataset = tracker.track_dataset(feature_data, "features", source="feature_engineering")
...
def track_ab_test_data(control_data, treatment_data, test_name, test_date):
    ...
    with mlflow.start_run(run_name=f"AB_Test_{test_name}_{test_date}"):
        ...
        if "conversion" in control_data.columns and "conversion" in treatment_data.columns:
