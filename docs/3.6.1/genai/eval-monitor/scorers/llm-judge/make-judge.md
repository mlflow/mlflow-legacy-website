...
The [make\_judge](/docs/3.6.1/api_reference/python_api/mlflow.genai.html#mlflow.genai.judges.make_judge) API is the recommended way to create custom LLM judges in MLflow. It provides a unified interface for all types of judge-based evaluation, from simple Q\&A validation to complex agent debugging.

Version Requirements

The `make_judge` API requires **MLflow >= 3.4.0**. For earlier versions, use the deprecated [custom\_prompt\_judge](/docs/3.6.1/api_reference/python_api/mlflow.genai.html#mlflow.genai.judges.custom_prompt_judge) instead.
...
To evaluate a batch of responses and track results in MLflow, use the [mlflow.genai.evaluate](/docs/3.6.1/api_reference/python_api/mlflow.genai.html#mlflow.genai.evaluate) function.
...
![Make Judge Example](/docs/3.6.1/images/mlflow-3/eval-monitor/scorers/make-judge-scorer.png)
...
| `trace`        | Trace is a special template variable which uses [agent-as-a-judge](/docs/3.6.1/genai/eval-monitor/scorers/llm-judge/agentic-overview.md). The judge has access to all parts of the trace. |
...
You can specify the type of the judge result using the required `feedback_value_type` argument. The [make\_judge](/docs/3.6.1/api_reference/python_api/mlflow.genai.html#mlflow.genai.judges.make_judge) API supports common types like `bool`, `int`, `float`, `str`, and `Literal` for categorical outcomes. This ensures judge LLMs produce structured outputs, making results reliable and easy to use.

## Versioning Scorers[​](#versioning-scorers "Direct link to Versioning Scorers")

To get reliable scorers, iterative refinement is necessary. [Tracking scorer versions](/docs/3.6.1/genai/eval-monitor/scorers/versioning.md) helps you maintain and iterate on your scorers without losing track of changes.
...
LLMs have biases and errors. Relying on biased evaluation will lead to incorrect decision making. Use [Automatic Judge Alignment](/docs/3.6.1/genai/eval-monitor/scorers/llm-judge/alignment.md) feature to optimize the instruction to align with human feedback, powered by the state-of-the-art algorithm from [DSPy](https://dspy.ai/).

## Next Steps[​](#next-steps "Direct link to Next Steps")

### [Evaluation Quickstart](/docs/3.6.1/genai/eval-monitor/quickstart.md)

[Get started with MLflow's evaluation framework.](/docs/3.6.1/genai/eval-monitor/quickstart.md)

[Start evaluating →](/docs/3.6.1/genai/eval-monitor/quickstart.md)

### [Collect Human Feedback](/docs/3.6.1/genai/assessments/feedback.md)

[Learn how to collect human feedback for evaluation.](/docs/3.6.1/genai/assessments/feedback.md)

[Collect feedback →](/docs/3.6.1/genai/assessments/feedback.md)

### [Aligning Judges with Human Feedback](/docs/3.6.1/genai/eval-monitor/scorers/llm-judge/alignment.md)

[Learn how to align your scorer with human feedback.](/docs/3.6.1/genai/eval-monitor/scorers/llm-judge/alignment.md)

[Learn alignment →](/docs/3.6.1/genai/eval-monitor/scorers/llm-judge/alignment.md)
