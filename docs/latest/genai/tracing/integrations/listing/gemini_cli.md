...
![Gemini CLI Tracing via OpenTelemetry](/docs/latest/assets/images/gemini-cli-tracing-412b3b9a961602fabd8a837641a08410.png)

[MLflow Tracing](/docs/latest/genai/tracing.md) provides automatic tracing for [Gemini CLI](https://github.com/google-gemini/gemini-cli), Google's open-source terminal-based coding agent. Gemini CLI has built-in OpenTelemetry support that sends rich conversation traces directly to MLflow's OTLP endpoint. Traces automatically capture:
...
MLflow automatically extracts token usage from Gemini CLI's GenAI semantic convention attributes and calculates cost based on the model. See the [Token Usage and Cost Tracking](/docs/latest/genai/tracing/token-usage-cost.md) documentation for details.
