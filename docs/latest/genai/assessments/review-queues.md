...
* To assign reviews using a user name, run the tracking server with [authentication](/docs/latest/self-hosting/security/basic-http-auth.md) enabled. On a server without authentication, reviews are attributed to a single `default` user and reviewer assignment is hidden, so use a [custom queue](#create-a-custom-queue) instead.
...
![Assigning traces to reviewers in the MLflow UI](/docs/latest/images/assessments/review-queue-assign-to-reviewers.png)
...
![The Review tab showing a selected queue and its traces](/docs/latest/images/assessments/review-queue-tab-overview.png)
...
![Reviewing a trace in the focused review page](/docs/latest/images/assessments/review-queue-focus-mode.png)
...
![Creating a review question in the MLflow UI](/docs/latest/images/assessments/review-queue-new-question.png)
...
![Creating a review queue in the MLflow UI](/docs/latest/images/assessments/review-queue-create-queue.png)
...
Other available calls: [`update_review_queue`](/docs/latest/api_reference/python_api/mlflow.genai.html#mlflow.genai.review_queues.update_review_queue) to change a custom queue's reviewers or questions, [`remove_items_from_review_queue`](/docs/latest/api_reference/python_api/mlflow.genai.html#mlflow.genai.review_queues.remove_items_from_review_queue) to detach traces, [`list_review_queues`](/docs/latest/api_reference/python_api/mlflow.genai.html#mlflow.genai.review_queues.list_review_queues) (filter by `user`) to find a reviewer's queues, and [`delete_review_queue`](/docs/latest/api_reference/python_api/mlflow.genai.html#mlflow.genai.review_queues.delete_review_queue) to delete a queue (the recorded reviewer answers are left untouched).
...
A review queue never stores answers separately from your traces. Each answer is written straight onto the reviewed trace (a **feedback** question logs feedback, an **expectation** question logs the expected ground-truth value), attributed to the reviewer. Re-saving an edited answer replaces the reviewer's previous one rather than creating a duplicate. This means review output is immediately available wherever trace data is used: the trace UI, [evaluation](/docs/latest/genai/eval-monitor.md), and ground-truth [datasets](/docs/latest/genai/datasets.md), with no extra export step.

## Next steps[​](#next-steps "Direct link to Next steps")

* **[Evaluate your app](/docs/latest/genai/eval-monitor.md)**: use the collected feedback and expectations as quality signals when you run evaluation.
* **[Align an LLM judge](/docs/latest/genai/eval-monitor/scorers/llm-judge/alignment.md)**: turn reviewer feedback into an aligned judge that scales human judgment to more traces.
* **[Build an evaluation dataset](/docs/latest/genai/datasets.md)**: promote ground-truth expectations into a dataset you can evaluate against.
