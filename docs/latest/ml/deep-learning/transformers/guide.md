...
When the model is then loaded with [`mlflow.pyfunc.load_model()`](/docs/latest/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.load_model), the prompt template will be used to format user inputs before passing them into the pipeline:
...
For a more in-depth guide, check out the [Prompt Templating notebook](/docs/latest/ml/deep-learning/transformers/tutorials/prompt-templating/prompt-templating.md)!
