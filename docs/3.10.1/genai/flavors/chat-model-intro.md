...
4. How [`ChatModel`](/docs/3.10.1/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.ChatModel) compares to [`PythonModel`](/docs/3.10.1/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.PythonModel) for defining custom chat models
...
* MLflow version 2.17.0 or higher installed for use of [`ChatModel`](/docs/3.10.1/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.ChatModel).
...
The [`mlflow.pyfunc.ChatModel`](/docs/3.10.1/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.ChatModel) interface sits between your application and MLflow's ecosystem, providing a layer of standardization that makes it easier to integrate your application with MLflow's other features and to deploy your model in an accessible, production-ready format.
...
![ChatModel Interface](/docs/3.10.1/assets/images/interface_1-eef793eaf139821002cfdb770dcd608d.png)
...
You can find details of the full chat response object [here](/docs/3.10.1/api_reference/python_api/mlflow.types.html#mlflow.types.llm.ChatCompletionResponse).
...
To demonstrate this mapping process, we will show how to use the [`mlflow.pyfunc.ChatModel`](/docs/3.10.1/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.ChatModel) class to log Meta's Llama 3.2 1B model via the Ollama llm client, which does not have a native MLflow flavor.
...
1. Define a class that extends [`mlflow.pyfunc.ChatModel`](/docs/3.10.1/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.ChatModel)
...
In a notebook environment, we can save the model to a file called `ollama_model.py` with the `%%writefile` magic command and call `set_model(SimpleOllamaModel())`. This is the "models from code" approach to model logging, which you can read more about [here](/docs/3.10.1/ml/model/models-from-code.md).
...
You can find the full list of supported parameters [here](/docs/3.10.1/api_reference/python_api/mlflow.types.html#mlflow.types.llm.ChatParams). Furthermore, you can pass arbitrary additional parameters to a ChatModel via the `custom_inputs` key in the input, which we will cover in more detail in the next section.
...
With a custom [`PythonModel`](/docs/3.10.1/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.PythonModel), we need to manually define the input example so that a model signature can be inferred using the example. This is a significant difference from the ChatModel API, which automatically configures a signature that conforms to the standard OpenAI-compatible input/output/parameter schemas. To learn more about auto inference of model signature based on an input example, see the [GenAI model signature example](/docs/3.10.1/ml/model/signatures.md#automatic-signature-inference) section for details.
...
* How [`ChatModel`](/docs/3.10.1/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.ChatModel) compares to the [`PythonModel`](/docs/3.10.1/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.PythonModel) for defining custom chat models
...
To learn more about these and other advanced features of the ChatModel API, you can read [this guide](/docs/3.10.1/genai/flavors/chat-model-guide.md).
