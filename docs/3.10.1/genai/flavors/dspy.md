...
![Overview of DSPy and MLflow integration](/docs/3.10.1/assets/images/dspy-integration-architecture-ee130c78236e38d9728f2a818bf024e9.png)
...
* [MLflow Tracking](/docs/3.10.1/ml/tracking.md) allows you to track your DSPy program's training and execution. With the MLflow APIs, you can log a variety of artifacts and organize training runs, thereby increasing visibility into your model performance.
* [MLflow Model](/docs/3.10.1/ml/model.md) packages your compiled DSPy program along with its dependency versions, input and output interfaces and other essential metadata. This allows you to deploy your compiled DSPy program with ease, knowing that the environment is consistent across different stages of the ML lifecycle.
...
[DSPy Quickstart](/docs/3.10.1/genai/flavors/dspy/notebooks/dspy_quickstart.md)

[Get started with MLflow and DSPy by exploring the simplest possible configuration of a DSPy program.](/docs/3.10.1/genai/flavors/dspy/notebooks/dspy_quickstart.md)
...
[](/docs/3.10.1/images/llms/tracing/dspy-tracing.mp4)

[MLflow Tracing](/docs/3.10.1/genai/tracing.md) tracing is a powerful feature that allows you to monitor and debug your DSPy programs. With MLflow, you can enable auto tracing just by calling the [`mlflow.dspy.autolog()`](/docs/3.10.1/api_reference/python_api/mlflow.dspy.html#mlflow.dspy.autolog) function in your code.
...
Learn more about MLflow DSPy tracing capabilities [here](/docs/3.10.1/genai/tracing/integrations/listing/dspy.md).
...
You can log the `dspy.Module` object to an MLflow run using the [`mlflow.dspy.log_model()`](/docs/3.10.1/api_reference/python_api/mlflow.dspy.html#mlflow.dspy.log_model) function.

We will also specify a [model signature](/docs/3.10.1/ml/model/signatures.md#automatic-signature-inference). An MLflow model signature defines the expected schema for model inputs and outputs, ensuring consistency and correctness during model inference.
...
![MLflow artifacts for the DSPy program](/docs/3.10.1/assets/images/dspy-artifacts-e5f816ebc8919118392f3e066edece68.png)

#### Loading the Module for inference[​](#loading-the-module-for-inference "Direct link to Loading the Module for inference")

The saved module can be loaded back for inference using the [`mlflow.pyfunc.load_model()`](/docs/3.10.1/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.load_model) function. This function gives an MLflow Python Model backed by the DSPy module.
...
To load the DSPy program itself back instead of the PyFunc-wrapped model, use the [`mlflow.dspy.load_model()`](/docs/3.10.1/api_reference/python_api/mlflow.dspy.html#mlflow.dspy.load_model) function.
...
The MLflow DSPy flavor supports autologging for DSPy optimizers. See the [Optimizer Autologging](/docs/3.10.1/genai/flavors/dspy/optimizer.md) page for details.
