...
![MLflow AI Gateway Tracing](/docs/3.10.1/images/llms/tracing/basic-openai-trace.png)
...
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------- |
| **Server-side Tracing** | Gateway automatically logs all requests when [usage tracking](/docs/3.10.1/genai/governance/ai-gateway/usage-tracking.md) is enabled | Centralized tracing for all requests through the gateway   |
| **Client-side Tracing** | Use OpenAI SDK with MLflow autolog                                                                                                   | Combining LLM traces with your agent or application traces |

When both are used together with a `traceparent` header, the gateway creates a linked span under the agent's trace for end-to-end visibility. See [Distributed Tracing](/docs/3.10.1/genai/tracing/app-instrumentation/distributed-tracing.md#ai-gateway-integration) for details.
...
Create an endpoint in MLflow AI Gateway to route requests to your LLM provider. See the [AI Gateway Quickstart](/docs/3.10.1/genai/governance/ai-gateway/quickstart.md) for detailed setup instructions.
...
Since MLflow AI Gateway exposes an OpenAI-compatible API, you can use MLflow's [OpenAI automatic tracing](/docs/3.10.1/genai/tracing/integrations/listing/openai.md) integration to trace calls.
...
You can use MLflow's [Anthropic automatic tracing](/docs/3.10.1/genai/tracing/integrations/listing/anthropic.md) integration to trace calls through MLflow AI Gateway.
...
You can use MLflow's [Gemini automatic tracing](/docs/3.10.1/genai/tracing/integrations/listing/gemini.md) integration to trace calls through MLflow AI Gateway.
...
You can use MLflow's [LangChain automatic tracing](/docs/3.10.1/genai/tracing/integrations/listing/langchain.md) integration to trace calls through MLflow AI Gateway.
...
### [Track User Feedback](/docs/3.10.1/genai/tracing/collect-user-feedback.md)

[Record user feedback on traces for tracking user satisfaction.](/docs/3.10.1/genai/tracing/collect-user-feedback.md)

[Learn about feedback →](/docs/3.10.1/genai/tracing/collect-user-feedback.md)

### [Manage Prompts](/docs/3.10.1/genai/prompt-registry.md)

[Learn how to manage prompts with MLflow's prompt registry.](/docs/3.10.1/genai/prompt-registry.md)

[Manage prompts →](/docs/3.10.1/genai/prompt-registry.md)

### [Evaluate Traces](/docs/3.10.1/genai/eval-monitor/running-evaluation/traces.md)

[Evaluate traces with LLM judges to understand and improve your AI application's behavior.](/docs/3.10.1/genai/eval-monitor/running-evaluation/traces.md)
