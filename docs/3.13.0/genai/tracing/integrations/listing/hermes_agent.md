...
Update `~/.hermes/config.yaml` so Hermes uses the [MLflow AI Gateway](/docs/3.13.0/genai/governance/ai-gateway.md) as the default model provider. With a small change to your configuration, you get centralized model routing for the same Hermes runs, plus:
...
See the [Hermes Agent + MLflow AI Gateway](/docs/3.13.0/genai/governance/ai-gateway/coding-agents/hermes-agent.md) guide to set this up.
...
![MLflow token usage and cost dashboard for Hermes Agent traces](/docs/3.13.0/images/llms/hermes/hermes-agent-token-usage.png)
