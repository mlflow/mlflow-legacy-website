...
[MLflow Tracing](/docs/3.12.0rc0/genai/tracing.md) provides automatic tracing capability for LangChain. You can enable tracing for LangChain by calling the [`mlflow.langchain.autolog()`](/docs/3.12.0rc0/api_reference/python_api/mlflow.langchain.html#mlflow.langchain.autolog) function, and nested traces are automatically logged to the active MLflow Experiment upon invocation of chains. In TypeScript, you can pass the MLflow LangChain callback to the `callbacks` option.
...
:::tip Image Support for LangChain Traces MLflow captures image content parts passed through LangChain models. See [Multimodal Content and Attachments in Traces](/docs/3.12.0rc0/genai/tracing/observe-with-traces/multimodal.md) for details. :::
...
When using LangChain.js with OpenTelemetry-based tracing, you can combine the automatically generated traces with the [MLflow Tracing SDK](/docs/3.12.0rc0/genai/tracing.md) (`@mlflow/core`) to add custom spans, set tags, and update trace metadata within the same trace.
